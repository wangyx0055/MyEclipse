...
public class SMSCTools
{
	...
	public static SubmitSM buildSubmitSMold(String srcAddr, String destAddr, String shortMsg, int seqNumber, int MsgType, String sCommand_code,
			int TotalSegments)
	{
		SubmitSM request = null;
		try
	...
	// delivery FAILURE
	public static SubmitSM buildSubmitSMold(String srcAddr, String destAddr, String shortMsg, int seqNumber, byte registeredDelivery, int MsgType,
			String sCommand_code, int TotalSegments)
	{
		...
		catch (Exception e)
		{
			...
			DBTools.ALERT("SMSCTools", "buildSubmitSMold", Constants.ALERT_SERIOUS, Preference.Channel + "Exception: " + e.getMessage(),
	...
	// = 2: SMSC Delivery Receipt requested where the final delivery outcome is
	// delivery FAILURE
	public static SubmitSM buildSubmitUTF8SM(String srcAddr, String destAddr, String shortMsg, int seqNumber, byte registeredDelivery, int MsgType,
			String sCommand_code, int TotalSegments)
	{
		SubmitSM request = null;
		try
	...
	public static DataSM buildDataSM(String srcAddr, String destAddress, int MsgType, String sCommand_code, int TotalSegments)
	{
		try
		{
			...
			if ((srcAddr.equals("996")) && (MsgType == 0)
					&& ((sCommand_code.equals("XSMN")) || (sCommand_code.equals("XSMT")) || (sCommand_code.equals("XSMB")) || (sCommand_code.equals("XSTD"))))
			{
				MsgType = 4;
			}
			if (TotalSegments == 0)
	...
	public static int CheckResend(int commandStatus, String MessageID)
	{
		...
		if (resend == 1)
			Gateway.util.log("SMSCTools", "{MessageID=" + MessageID + "}{ERR=" + strTemp + ":" + commandStatus_Hex + "{Resend=Yes}");
		else
		{
			Gateway.util.log("SMSCTools", "{MessageID=" + MessageID + "}{ERR=" + strTemp + ":" + commandStatus_Hex + "{Resend=No}");

		}

		if (alert == 1)
		{
			DBTools.ALERT("CheckResend", "CheckResend", Constants.ALERT_WARN, "{ERR=" + strTemp + ":" + commandStatus_Hex + "{Resend=" + resend + "}",
					Preference.ALERT_CONTACT);
		}
	...
	public static SubmitSM buildSubmitSM(String srcAddr, String destAddr, String shortMsg, int seqNumber, byte registeredDelivery, int MsgType,
			String sCommand_code, BigDecimal RequestID)
	{
		SubmitSM request = null;
		try
		{
			...
			Logger.info("SMSCTools.buildSubmitSM", "{RequestID=" + RequestID + "}{srcAddr=" + srcAddr + "}{destAddr=" + destAddr + "}{setDestSubaddress="
					+ DestSubAddress + "} {chargeflag=" + chargeflag + "}{servicetype=" + servicetype + "} ");
			return request;
		}
		catch (Exception e)
		{
			...
			DBTools.ALERT("SMSCTools", "buildSubmitSM.Exception", Constants.ALERT_SERIOUS, Preference.Channel + "Exception: " + e.getMessage(),
					Preference.ALERT_CONTACT);

			return null;
		}
	}
	public static SubmitSM buildLongSubmitSM(String srcAddr, String destAddr, String shortMsg, int seqNumber, byte registeredDelivery, int MsgType,
			String sCommand_code, BigDecimal RequestID)
	{
		SubmitSM request = null;
		try
		{
			...
			message.appendCString(shortMsg);
