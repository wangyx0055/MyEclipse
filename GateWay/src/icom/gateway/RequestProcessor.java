...
public class RequestProcessor extends Thread
{
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
			...
			{ // when lost connection to db
				...
				DBTools.ALERT("RequestProcessor", "RequestProcessor", Constants.ALERT_WARN, Preference.Channel + "DBException: " + ex.getMessage(),
			...
			catch (Exception e)
			{
				...
				DBTools.ALERT("RequestProcessor", "RequestProcessor", Constants.ALERT_WARN, Preference.Channel + "Exception: " + e.getMessage(),
						Preference.ALERT_CONTACT);
			}
	...
	private void processRequest(PDU pdu) throws DBException, IOException
	{
		...
		if (pdu.getCommandId() == Data.DELIVER_SM)
		{
			...
			catch (ValueNotSetException e)
			{
				...
				try
				{
					...
					if (udhlength == 6 && udhele1 == 5)
					{
						destport = sdport;
						this.info = newmsg;
						Gateway.util.log(this.getClass().getName(), "getDestinationPort:" + destport + "@info=" + newmsg);
					}
			...
			if (dsm.getEsmClass() == 0x4 || info.startsWith("id:"))
			{ // DeliverReport
				// (not
				// processed)!
			...
			dbTools.add2SMSReceiveQueueR(this.userId, this.serviceId, this.operator, this.commandCode, this.info, this.RequestID, this.Dport);
	...
	private int checkMobileBuffer(String mobile)
	{
		...
		{ // not exist --> create new
			MobileBuffer.add(mobile, new MobileBufferInfo(mobile, currTime, 0, 1, 0, 0));
			return 1; // OK
		}
		// EXIST --> update
		int result = 0;
		...
		if (lastTime <= 0)
		{ // added by MT thread (no MO before) --> update
			info.mo_Counter = 1; // first MO
			result = 1; // OK
		}
		else if (currDay != getDay(lastTime * 1000))
		{ // new day/date
			// //////////////////////////////////////////////////////////
	...
	private String buildMobileOperator(String userId)
	{
		...
		else if (temp.startsWith("8490") || temp.startsWith("+8490") || temp.startsWith("8493") || temp.startsWith("+8493"))
		{
			result = "VMS";
		}
		else if (temp.startsWith("8491") || temp.startsWith("+8491") || temp.startsWith("8494") || temp.startsWith("+8494"))
		{
			result = "GPC";
		}
		else if (temp.startsWith("8498") || temp.startsWith("+8498") || temp.startsWith("8497") || temp.startsWith("+8497"))
		{
			result = "VIETTEL";
		}
