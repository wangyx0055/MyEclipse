...
public class DBTools
{
	...
	public static int add2SMSReceiveQueueR(String userId, String serviceId, String mobileOperator, String commandCode, String info, String RequestID,
			String DestinationPort) throws DBException
	{
		...
		try
		{
			...
			conn = getConnection(Preference.LIST_DB_RECEIVE_QUEUE[Preference.getIDdbReceiveQueue(userId)]);

			Timestamp time = new Timestamp(System.currentTimeMillis());
			int NOT_RESPONDED = 0;

			strSQL = "INSERT INTO sms_receive_queue (USER_ID, SERVICE_ID, " + "MOBILE_OPERATOR, COMMAND_CODE, INFO, RECEIVE_DATE, RESPONDED,REQUEST_ID,DPORT) "
					+ "VALUES ( ?, ?, ?, ?, ?, ?, ?,?,?)";
			...
			if (preStmt.executeUpdate() != 1)
			{
				...
				DBTools.ALERT("DBTools", "add2SMSReceiveQueue", Constants.ALERT_WARN, Preference.Channel + "@Error adding row", Preference.ALERT_CONTACT);
			...
			else
			{
				...
				Logger.info("DBTools.add2SMSReceiveQueue", "{Add-MO2DB}{Request_ID=" + RequestID + "}{User_ID=" + userId + "}{Service_ID=" + serviceId
						+ "}{Info=" + info + "}");
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2SMSReceiveQueue", Constants.ALERT_WARN, Preference.Channel + "@Error adding row:" + e.getMessage(),
	...
	public static int ALERT(String domain, String issue, String level, String alertmsg, String contact)
	{
		...
		Logger.info("ALERT:" + "{domain=" + domain + "}{issue=" + issue + "}{level=" + level + "}{domain=" + level + "}{alertmsg=" + alertmsg + "}{contact="
				+ contact + "}");
		...
		if (Preference.ALERT == 1)
		{
			...
			String sSQL = "insert into msg_alerter( domain, issue, level,alertmsg,contact) " + "values(?,?,?,?,?)";
			try
			{
				if (connection == null)
					connection = getConnection(Preference.LIST_DB_ALERT[0].trim());
	...
	public boolean getAllEMSSendQueue(String sRound, String iMod, String iNum, Queue Ems, int idDB) throws DBException
	{
		DBException exception = null;
		java.sql.Connection conn = null;
		PreparedStatement preStmt = null;
		...
		try
		{
			...
			strSQL = "select id, user_id, service_id, mobile_operator, command_code, content_type, " + "info,  process_result, message_type, "
					+ "request_id, message_id, total_segments,cpid " + "from ems_send_queue" + sR + " where  info is not null " + "and (mod(id," + iMod
					+ ") = " + iNum + ") and upper(mobile_operator)=upper('" + Preference.mobileOperator + "') and service_id in ("
					+ Preference.strLIST_SERVICEID + ")";

			preStmt = conn.prepareStatement(strSQL, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
			...
			if (preStmt.execute())
			{
				...
				while (rs.next())
				{
					...
					if (Preference.CheckServiceId(serviceid) & Gateway.bound)
					{
						...
						{
							keyword = Gateway.loadconfig.getKeyword(commandcode, serviceid);
						...
						try
						{
							Logger.info(this.getClass().getName(), "{getMTfromDB}{Request_ID=" + ems.getRequestId() + "}{User_ID=" + ems.getUserId() + "}");
							rs.deleteRow();
							Ems.enqueue(ems);
						...
						{
							Logger.error("DBTools", "{getAllEMSSendQueue: Error executing SQL }{getMTfromDB}{Request_ID=" + ems.getRequestId() + "}{User_ID="
									+ ems.getUserId() + "}" + strSQL + ": " + ex.toString());
							DBTools.ALERT("DBTools", "getAllEMSSendQueue", Constants.ALERT_WARN, Preference.Channel + "@getAllEMSSendQueue:" + ex.getMessage(),
									Preference.ALERT_CONTACT);
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "getAllEMSSendQueue", Constants.ALERT_WARN, Preference.Channel + "@getAllEMSSendQueue:" + e.getMessage(),
					Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
	...
	public int add2EMSSendLog(EMSData ems)
	{
		...
		try
		{
			...
			strSQL = "insert into " + tablename + " ( USER_ID, SERVICE_ID, MOBILE_OPERATOR, " + "COMMAND_CODE, CONTENT_TYPE, INFO, SUBMIT_DATE, DONE_DATE, "
		...
		{
			Logger.error(this.getClass().getName(), "SQLException: Error executing SQL " + strSQL + ">>>" + e.getMessage());
			Gateway.saveSMSObject(ems.getId() + ".slog", ems);
			DBTools.ALERT("DBTools", "add2EMSSendLog", Constants.ALERT_WARN, Preference.Channel + "@add2EMSSendLog:" + e.getMessage(), Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
			return result;
		}
		catch (Exception e)
	...
	public int add2BlacklistLog(EMSData ems)
	{
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2BlacklistLog", Constants.ALERT_WARN, Preference.Channel + "@add2BlacklistLog:" + e.getMessage(),
					Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
			return result;
		}
	...
	public static boolean add2CdrQueueb(EMSData ems, String MessageType) throws DBException
	{
		...
		try
		{
			...
			strSQL = "insert into cdr_queue ( USER_ID, SERVICE_ID, " + "MOBILE_OPERATOR, COMMAND_CODE, INFO, "
					+ "SUBMIT_DATE, DONE_DATE, TOTAL_SEGMENTS,Message_Type,process_result,request_id,cpid) " + "values (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";

			preStmt = conn.prepareStatement(strSQL);
			preStmt.setString(1, ems.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL));
			preStmt.setString(2, ems.getServiceIdEx(Constants.SERVICEID_FORMAT_INTERNATIONAL));
			preStmt.setString(3, ems.getMobileOperator());
			preStmt.setString(4, ems.getCommandCode());
			...
			else
			{
				...
				DBTools.ALERT("DBTools", "add2BlacklistLog", Constants.ALERT_WARN, Preference.Channel + "@Cannot add CDRQueue", Preference.ALERT_CONTACT);
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2CdrQueueb", Constants.ALERT_WARN, Preference.Channel + "@add2CdrQueueb:" + e.getMessage(), Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
	...
	public static boolean add2CdrLogb(EMSData ems, String MessageType) throws DBException
	{
		DBException exception = null;
		boolean rt = false;
		java.sql.Connection conn = null;
		...
		try
		{
			...
			strSQL = "insert into cdr_log" + new SimpleDateFormat("yyyyMM").format(new Date()) + " ( USER_ID, SERVICE_ID, "
					+ "MOBILE_OPERATOR, COMMAND_CODE, INFO, " + "SUBMIT_DATE, DONE_DATE, TOTAL_SEGMENTS,Message_Type,process_result,request_id,cpid) "
					+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
