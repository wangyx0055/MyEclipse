...
public class ResendErrMt2Telcos extends Thread
{
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
				...
				if (ResendQueue.size() > 0)
				{
					for (int i = 0; i < ResendQueue.size(); i++)
					{
						...
						if (sms != null)
							...
							Gateway.util.log(this.getClass().getName(), "{Can't resend sms get from ResendQueue is null}");
						this.sleep(100);
					}
			...
			{
				Gateway.util.logErr(this.getClass().getName(), "ResendErrMt2Telcos:" + ex.getMessage());
			}
		}
		// /////////////////////////////
		this.destroy();
	...
	public void ResendMT(EMSData sms)
	{
		try
		{
			...
			{
				Gateway.util.log(this.getClass().getName(), "{ResendMT:add2 SendQueue}{User_ID=" + sms.getUserId() + "}{Service_ID=" + sms.getServiceId()
						+ "}{Info=" + sms.getText() + "}{Request_ID" + sms.getsRequestID() + "}");
		...
		{
			Gateway.util.logErr(this.getClass().getName(), "{ERR: ResendMT}{User_ID=" + sms.getUserId() + "}{Service_ID=" + sms.getServiceId() + "}{Info="
