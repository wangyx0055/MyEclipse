		...
		if (pdu.getCommandId() == Data.SUBMIT_SM_RESP)
		{
			...
			if (commandStatus == Data.ESME_ROK)
			{
				...
				synchronized (wait4ResponseTable)
				{
					...
					if (ems == null)
					{
						Gateway.util.logErr(this.getClass().getName(), "{wait4ResponseTable: ems==null}{emsId=" + ssmr.getSequenceNumber() + "}{MessageID="
								+ ssmr.getMessageId() + "}");
						return;
					}
			...
			else
			{
				...
				{ // =1: response for Invalid message
					Gateway.util.log(this.getClass().getName(), "ResponseProcessor: seqNumber < 10, =" + this.seqNumber);
				...
				synchronized (wait4ResponseTable)
				{
					...
					if (ems == null)
					{
						Gateway.util.log(this.getClass().getName(),
								"{processResponse: ems==null}{emsId=" + this.seqNumber + "}{MessageID=" + ssmr.getSequenceNumber() + "}");
						return;
					}
					ems.setNotes(ems.getNotes() + ":" + sErr);
					Gateway.util.log(this.getClass().getName(), "{Response for MT}{Request_ID=" + ems.getRequestId() + "}{User_ID=" + ems.getUserId()
							+ "}{Message_ID=" + ems.getMessageId() + "}");
					// Last segment ?
					...
					if ((resend == 1) && (ems.getSendNum() < Preference.NumOfRetries))
					{
						...
						Gateway.util.log(
								...
								"{Add MT to Resend Queue}{Request_ID=" + ems.getRequestId() + "}{User_ID=" + ems.getUserId() + "}{Message_ID="
										+ ems.getMessageId() + "}{Resend num=" + ems.getSendNum() + "}");
						ResendQueue.enqueue(ems);
					}
					else
					{
						Gateway.util.log(this.getClass().getName(), "{MT not Resend}{Request_ID=" + ems.getRequestId() + "}{User_ID=" + ems.getUserId()
								+ "}{Message_ID=" + ems.getMessageId() + "}{Resend num=" + ems.getSendNum() + "}");
