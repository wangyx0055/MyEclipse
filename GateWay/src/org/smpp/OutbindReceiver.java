    ...
    protected void receiveAsync()
    {
        ...
        if (connection != null) {
            ...
            // in uncontrolled manner
                ...
                Logger.error( "Buffer didn't contain enough data, continuing.");
            } catch (UnknownCommandIdException e) {
            	Logger.error( "Unknown command id, continuing.");
            } catch (TimeoutException e) {
                Logger.verbose("Timeout expired, message not received completly in time.");
            } catch (TLVException e) {
            	Logger.error("Wrong TLV");
            } catch (PDUException e) {
            	Logger.error( "Wrong PDU");
            } catch (Exception e) {
            ...
            if (pdu != null) {
                if (pdu.getCommandId() == Data.OUTBIND) {
                    Logger.verbose("Got outbind PDU, sending event info. " + pdu.debugString());
                    sendOutbindEvent(new OutbindEvent(this, connection, (Outbind)pdu));
                } else {
                    Logger.verbose("PDU isn't outbind, throwing away. " + pdu.debugString());
