...
public class Common
{
	...
	public static int sendMT1(MsgObject msgObject)
	{
		...
		try
		{
			...
			{
				Result = mSendQueue.Insert_VMS(idseq, msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(), msgObject.getKeyword(),
			...
			{
				Result = mSendQueue.Insert(idseq, msgObject.getUserid(), msgObject.getServiceid(), msgObject.getMobileoperator(), msgObject.getKeyword(),
					msgObject.getUsertext(), null, null,  Integer.toString(msgObject.getMsgtype()), msgObject.getRequestid().toString(), "1",
	...
	public static SessionObject GetCurrentSession() throws Exception
	{
		try
		{
			if (ConsoleSRV.ListSessionObject.size() > 0)
				return ConsoleSRV.ListSessionObject.get(0);
			else
