...
public class Textbaserandom_multi extends DeliveryManager_multi {
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			String sqlSelect = "select id, user_id, service_id, last_code,command_code,request_id, message_type,mobile_operator,mt_count,mt_free,duration,TIMESTAMPDIFF(day, date," + Constants.PROMO_DATE  + ")  from "
					+ MLIST
			...
			for (int i = 0; i < vtUsers.size(); i++) {
				...
				String[] arrContents = getContent(INFO_ID, lastcode);
