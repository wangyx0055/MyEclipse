...
public class Infoservices extends DeliveryManager {
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			try {
				...
				DBUtil.Alert("Process.VMS", "infoservices", "major",
						"infoservices.Exception: co loi khi get service.ex="
								+ ex.toString(), "");
			}

			// gui mt den nhung thue bao vua moi dang ky
			SendMT2UserHaveJustRegister(ssid, serviceName, MLIST, DBCONTENT,
					type, CLASSNAME, notcharge, INFO_ID, strResult, service);
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
			...
			int notcharge, String INFO_ID, String sSplit, String strInfo)
			throws Exception {
		Util.logger
				.info("@Infoservices@SendMT2UsersHasCharged: lay danh sach thue bao charge offline - table list send,service name="
						+ serviceName);

		Vector vtUsers = DBUtil.getListUserFromListSend(serviceName);
		int cUser = vtUsers.size();
		Util.logger.info("@Infoservices@SendMT2UsersHasCharged: vtUsers="
				+ cUser);
	...
	private void SendMT2UserHaveJustRegister(String ssid, String serviceName,
			String MLIST, String DBCONTENT, String type, String CLASSNAME,
			int notcharge, String INFO_ID, String strInfo, icom.Services service)
			throws Exception {
		Util.logger
				.info("@Infoservices@SendMT2UserHaveJustRegister: lay danh sach thue bao charge online- table mlist="
						+ MLIST + ",service name=" + serviceName);

		Vector vtUsers = DBUtil.getListUserFromMlist(serviceName, MLIST);
		int cUser = vtUsers.size();
		Util.logger.info("@Infoservices@SendMT2UserHaveJustRegister: vtUsers="
				+ cUser);
		...
		for (int i = 0; i < vtUsers.size(); i++) {
			...
			if (DBUtil
					.sendVMSChargeOnline(type, msgObj, CLASSNAME, serviceName) == 1) {
				if (duration > 0 && (mtcount > duration)) {
					// xoa thoi
					DBUtil.insertData2cancel(userid, serviceid, commandcode,
							MLIST, msgObj, mtfree + "", msgtype, commandcode,
							mtcount + "");
	...
	private String getContent2(String dbcontent, String type, String infoid,
		...
		try {
			...
			Util.logger
					.info("Infoservices: Sql select content : sql =" + query);
