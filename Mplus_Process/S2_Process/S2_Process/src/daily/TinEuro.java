...
public class TinEuro  extends DeliveryManager{
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
		...
		Util.logger.info("@TinVanEuro@SendMT2UsersHasCharged: lay danh sach thue bao charge offline - table list send,service name="+ serviceName);
		Vector vtUsers = DBUtil.getListUserFromListSend(serviceName);
		String strInfo = getContent0(serviceName);
		int cUser = vtUsers.size();
		Util.logger.info("@TinVanEuro@SendMT2UsersHasCharged: vtUsers="+ cUser);
		for (int i = 0; i <cUser; i++) {
			...
			Util.logger.info("TinVanEuro@Noi dung tra ve cho khach hang: "
					+ content);
				...
				DBUtil
						.executeSQL(
								...
								"update "
										...
										+ userid + "'");
				// return null;
				DBUtil.Alert("TinVanEuro@Infoservices", "RUNING", "major",
	...
	public String getContent0(String serviceName) {
		...
		String table ="";
		if(serviceName.equals("TVEU")) table = "TextNewsEuro";
		if(serviceName.equals("KQEU")) table = "TextResult";
		if(serviceName.equals("TVEU")) table = "TextSchedule";
		try {

			connection = dbpool.getConnection("CMSBinhLuanS2");

			String query = " select TOP 1 Content ,subcode,returndate " +
							" from "+table+" where returndate <= getdate() AND status = 2 " +
							" ORDER BY returndate DESC ";

			Util.logger.info("TINVANEURO: Sql select content : sql =" + query);

			Vector result = DBUtil.getVectorTable(connection, query);
			if (result.size() == 0) {
				Util.logger.error("TINVANEURO: Cannot get content");
				return null;
			} else {
