...
public class TextBaseRandomNew  extends DeliveryManager{
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			{
				Util.logger.error("TextBaseRandomNew: co loi khi get service");
				DBUtil
				.Alert(
						"Process.VMS",
						"TextBaseRandomNew",
						"major",
						"TextBaseRandomNew.Exception: co loi khi get service.ex=" + ex.toString(),"");
			}
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
		...
		for(int i = 0; i< arrLastCode.size(); i++){
			...
			for(int j = 0;j<vtUsers.size();j++){
				...
				Util.logger.info("TextBaseRandomNew@Noi dung tra ve cho khach hang: " + content);
			...
			Util.logger.info("TextBaseRandomNew: charge offline: update last code trong mlist. \tmlist:" + MLIST + "\tlastcode:" + lastcode+ "\tcommand_code:" + serviceName);
	...
	private void SendMT2UserHaveJustRegister(String ssid, String serviceName,
		...
		Util.logger.info("@TextBaseRandomNew@SendMT2UserHaveJustRegister: lay danh sach thue bao charge online- table mlist="  + MLIST + ",service name=" + serviceName);
		Vector vtUsers = DBUtil.getListUserFromMlist(serviceName, MLIST);
		
		int cUser = vtUsers.size();
		Util.logger.info("@TextBaseRandomNew@SendMT2UserHaveJustRegister: vtUsers="+ cUser);
		...
		for (int i = 0; i < cUser; i++) {
			...
			// failures: khong gui tin cho KH vi content bi null
			Util.logger.info("TextBaseRandomNew@SendMT2UserHaveJustRegister@latsid=" + lastid + ",content=" + content);
			if ("".equalsIgnoreCase(lastid) || "".equalsIgnoreCase(content)) {
				DBUtil
						.executeSQL(
								"gateway",
			...
			Util.logger.info("TextBaseRandomNew\tSendMT2UserHaveJustRegister\tSend vms_charge_online");
			if (DBUtil.sendVMSChargeOnline(type, msgObj, CLASSNAME, serviceName) == 1) {
				Util.logger.info("TextBaseRandomNew: charge online: update last code trong mlist. \tmlist:" + MLIST + "\tuser_id:" + userid+ "\tcommand_code:" + serviceName);
				DBUtil
						.executeSQL(
								"gateway",
								"update "
	...
	public String[] getContent0(String database, String type, String lastcode,
		...
		query = query + " and id not in (" + lastcode + ")order by id  )x ";
		Util.logger.info("TextBaseRandomNew: Sql select content: Sql = " + query);
		try {
	...
	public ArrayList<SameLastCodeObject> getGroupLastCode(String commandCode){
		...
		try {
			...
			Util.logger.error("TextBaseRandomNew - getGroupLastCode. SQLException:"
					+ ex3.toString());
