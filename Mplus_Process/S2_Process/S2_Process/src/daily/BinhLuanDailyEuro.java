...
public class BinhLuanDailyEuro extends DeliveryManager{
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			String INFO_ID = "x";
			String MLIST = "x";
			String CLASSNAME = "BinhLuanDailyEURO";
			...
			Util.logger.info("Ngay gio: " + currDate + "; tra tin dich vu " + serviceName);
			...
			}catch(Exception ex)
			{
				...
				.Alert(
						...
						"infoservices",
						"major",
						"BinhLuanDaily.Exception: co loi khi get service.ex=" + ex.toString(),"");
			}
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
		...
		Util.logger.info("@BinhLuanDaiLyEuro@SendMT2UsersHasCharged: lay danh sach thue bao charge offline - table list send,service name="+ serviceName);
		
		Vector vtUsers = DBUtil.getListUserFromListSend(serviceName);
		int cUser = vtUsers.size();
		Util.logger.info("@BinhLuanDaiLyEuro@SendMT2UsersHasCharged: vtUsers="+ cUser);
	...
	private String getContent2(String dbcontent, String type, String infoid,
		...
		try {
			...
			String query = " select TOP 1 Content ,subcode,returndate " +
							" from TextComment where returndate <= getdate() AND status = 2 " +
							" ORDER BY returndate DESC ";

			Util.logger.info("BinhLuanDaily: Sql select content : sql =" + query);

			Vector result = DBUtil.getVectorTable(connection, query);
			if (result.size() == 0) {
				Util.logger.error("BinhLuanDaily: Cannot get content");
				return null;
			} else {
