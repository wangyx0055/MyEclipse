...
public class Textbasedovui extends DeliveryManager {

	String CLASSNAME = "daily.Textbasedovui";
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			STYPE = getStringfromHashMap(_option, "type", "1");
			...
			String sqlSelect = "select id, user_id, service_id, last_code,command_code,request_id, message_type,mobile_operator,mt_count,mt_free,duration,TIMESTAMPDIFF(day, date," + Constants.PROMO_DATE  + "),last_id  from "
					+ MLIST
			...
			for (int i = 0; i < vtUsers.size(); i++) {
				...
				if (notcharge == Constants.MODE_NOTCHARGE || "0".equalsIgnoreCase(STYPE)) {
					msgtype = Integer.parseInt(Constants.MT_PUSH);
				}
				
				String lastidquest = (String) item.elementAt(12);

				String[] arrContents = getContent(INFO_ID, lastcode,STYPE,lastidquest);
						...
						DBUtil
								.executeSQL(
										...
										"update "
												...
												+ " set last_code = '"
												+ lastcode
												+ "' ,autotimestamps = current_timestamp,mt_count=mt_count+1,failures=0,last_id='" + lastid  +"' where id ="
												+ id);
					}
				} else {
					DBUtil
	...
	String[] getContent(String gameid, String lastcode,String stype,String lastid) {
