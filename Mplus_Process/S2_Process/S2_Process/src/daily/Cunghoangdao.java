...
public class Cunghoangdao extends DeliveryManager {
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			for (int i = 0; i < arrCung.length; i++) {
				...
				arrInfo[i] = getContent2(DBCONTENT, type, arrCung[i], currDate);
				if (arrInfo[i] == null) {
					Util.logger.error("@Cung hoang dao: khong doc duoc content!. Kiem tra ket noi hoac noi dung?");
					DBUtil.Alert("DeliveryDaily", "RUNING", "major",
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
		...
		QueueListSendMng queueListSend = QueueListSendMng.getInstance();
		
		while(Sender.getData){
			
			ListSendObj obj = queueListSend.getObjResponseMT(serviceName);
			if(obj == null){
				break;
			}
			
			String options = (String) obj.getOptions();
			...
			if (queueListSend.insertMTQueue(obj, content) == 1) {
				DBUtil.executeSQL(
								"gateway",
								"update "
										+ MLIST
										...
										+ obj.getUserId() + "'");
				/******
				 * Update status_mt trong list send
				 * **/
				queueListSend.updateStatusMT(obj.getId(), 1);
	...
	private String getContent2(String dbcontent, String type, String infoid,
		...
		try {
			...
			String query = "SELECT "
					+ content
					+ " FROM (SELECT TOP 1 * FROM horoscope_info WHERE status=1 and upper(horotypecode) = '"
					+ infoid.toUpperCase()
					+ "' AND CONVERT(varchar(25), [isdate], 103) ='" + sDate
					+ "')x";

			Util.logger.info("Cunghoangdao: getcontent2: lay noi dung: sql:" + query);
