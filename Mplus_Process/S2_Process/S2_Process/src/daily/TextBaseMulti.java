...
public class TextBaseMulti extends DeliveryManager {
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			{
				Util.logger.error("TextBaseMulti: co loi khi get service");
				DBUtil
				.Alert(
						"Process.VMS",
						"TextBaseMulti",
						"major",
						"TextBaseMulti.Exception: co loi khi get service.ex=" + ex.toString(),"");
			}
			...
			String sqlUpdate = "update services set result="
					...
					+ ", lasttime=current_timestamp() where id=" + ssid;
			DBUtil.executeSQL("gateway", sqlUpdate);
			DBUtil.Alert("DeliveryDaily@TextBaseRandomNew", "RUNING", "major",
	...
	private void SendMT2UsersHasCharged(String ssid, String serviceName,
		...
		Util.logger.info("@TextBaseRandomNew@SendMT2UsersHasCharged: lay danh sach thue bao charge offline - table list send,service name="+ serviceName);
		...
		for(int i = 0; i< arrLastCode.size(); i++){
			...
			for(int j = 0;j<vtUsers.size();j++){
				...
				Util.logger.info("TextBaseMulti@Noi dung tra ve cho khach hang: " + content);
				...
				if ("".equalsIgnoreCase(lastid) || "".equalsIgnoreCase(content)) {
					...
					// return null;
					DBUtil.Alert("DeliveryDaily@TextBaseMulti", "RUNING", "major",
			...
			Util.logger.info("TextBaseMulti: charge offline: update last code trong mlist. \tmlist:" + MLIST + "\tlastcode:" + lastcode+ "\tcommand_code:" + serviceName);
			...
			Util.logger.info("SQL Update Last Code table : "+MLIST);
			Util.logger.info(" Sql :" + sqlUpdateLastCode);

			DBUtil.executeSQL("gateway", sqlUpdateLastCode);
			
			String sqlUpdateLastCodeListSend = "UPDATE list_send SET last_code = '"+ lastcode 
												+"' WHERE command_code ='" + serviceName + "' ";
			
			Util.logger.info("SQL Update Last Code table : list_send");
			Util.logger.info("SQL Update Last Code : " + sqlUpdateLastCodeListSend);
			
			DBUtil.executeSQL("gateway", sqlUpdateLastCodeListSend);
		}
	}
	...
	private void SendMT2UserHaveJustRegister(String ssid, String serviceName,
		...
		Util.logger.info("@TextBaseMulti@SendMT2UserHaveJustRegister: lay danh sach thue bao charge online- table mlist="  + MLIST + ",service name=" + serviceName);
		Vector vtUsers = DBUtil.getListUserFromMlist(serviceName, MLIST);
		
		int cUser = vtUsers.size();
		Util.logger.info("@TextBaseMulti@SendMT2UserHaveJustRegister: vtUsers="+ cUser);
		...
		for (int i = 0; i < cUser; i++) {
			...
			// failures: khong gui tin cho KH vi content bi null
			Util.logger.info("TextBaseMulti@SendMT2UserHaveJustRegister@latsid=" + lastid + ",content=" + content);
			if ("".equalsIgnoreCase(lastid) || "".equalsIgnoreCase(content)) {
				DBUtil
						.executeSQL(
								"gateway",
								"update "
										...
										+ userid + "'");
				// return null;
				DBUtil.Alert("DeliveryDaily@TextBaseRandomNew@SendMT2UserHaveJustRegister", "RUNING", "major",
						"Kiem tra dich vu: content bi null. de nghi nhap content ngay."
								+ serviceName + "", "");
			}
			if (lastcode.length() > 3500) {
			...
			Util.logger.info("TextBaseMulti\tSendMT2UserHaveJustRegister\tSend vms_charge_online");
			if (DBUtil.sendVMSChargeOnline(type, msgObj, CLASSNAME, serviceName) == 1) {
				Util.logger.info("TextBaseMulti: charge online: update last code trong mlist. \tmlist:" + MLIST + "\tuser_id:" + userid+ "\tcommand_code:" + serviceName);
				DBUtil
						.executeSQL(
								"gateway",
								"update "
	...
	/**
	 * Funtion get content form database cms ( sql server),return cateid and content for send MT to customer
	 * @param database
	 * @param type
	 * @param lastcode
	 * @param newstypecode
	...
	public String[] getContent0(String database, String type, String lastcode,
		...
		// Xac dinh truong nao can lay du lieu
		...
		String query = "SELECT id,"
			+ content
			+ " FROM ( SELECT TOP 1 * FROM [Contents] WHERE status=2 and cateid = (select cateid from [Cate]  where upper(prefixcode) like '"+newstypecode.toUpperCase()+"')"; 
		query = query + " and id not in (" + lastcode + ") and DATEDIFF(day, returndate, GETDATE()) = 0 order by returndate )x ";
		try {
	...
	/**
	 * Function get old code group from talbe list_send 
	 * @param commandCode
	 * @return
	 */
	public ArrayList<SameLastCodeObject> getGroupLastCode(String commandCode){
		...
		try {
			...
			Util.logger.error("TextBaseMulti - getGroupLastCode. SQLException:"
					+ ex3.toString());
