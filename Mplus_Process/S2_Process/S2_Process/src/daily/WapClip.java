...
import daily.WapPicture.SubThreadDeliveryPicture;

import resend.ResendCommon;
import soap.sendXMLGAME;
import sub.DeliveryManager;
import vmg.itrd.ws.MTSenderVMS;

public class WapClip extends DeliveryManager {
	...
	public static String CLASSNAME = "daily.WapClip";
...
//					type, CLASSNAME, notcharge, INFO_ID, typehd, GID, service);
	...
	private void SendMT2UsersHasCharged(String ssid, String DBCONTENT,
		...
		for(int i = 0;i<numberThread;i++){
			SubThreadDeliveryClips thDelivery = 
				new SubThreadDeliveryClips(serviceName,i);
			thDelivery.start();
		}			
	...
	private void SendMT2UserHaveJustRegister(String ssid, String DBCONTENT,
		...
		for (int i = 0; i < cUser; i++) {
			...
			try {
				...
				result = linkMng.getLinkClip(code, now(), userid, dAmount);
			...
			if ((result.startsWith("0"))
				...
				Util.logger.info("@Lay link thanh cong:image[request_id:"
						...
						+ "] charge online : Duong link tai clip: " + temp);
				...
				if (mtqueue_gamering.equalsIgnoreCase("1")) {
					if (insertMtqueue(userid, namecode + ":" + temp, serviceid,
						...
						Util.logger
								.info("@image[request_id:"
										+ msgObject.getRequestid()
										+ "] charge online : Luu thong tin image vao db - icom_game: user_id:"
										+ userid + "\tcode:" + code + "\tlink:"
										+ temp);
						Util.CallThread(new COMObject(new GameInfo(namecode,
								code + "", temp, userid)), 4);
					} else {
						Util.logger
								.info("@image[request_id:"
	...
	class SubThreadDeliveryClips extends Thread {
		...
		public SubThreadDeliveryClips(String _serviceName, int _numberThread) {
		...
		public void run() {
			...
			while (Sender.processData) {
				...
				if(obj == null){
					...
					if(numberThread == 0){
						Util.logger.info("DELIVERY CLIP: "+ serviceName +" FINISH!!!");
						String sqlUpdate = "update services set result="
				...
				if (temp.trim().equals("")) {
					...
					String result = getLinkMng.getLinkClip(mediaId, now(), obj.getUserId(), amount);

					if(result == null){
						Util.logger.error("SubThreadDeliveryClips ERROR ## link result = null; " +
