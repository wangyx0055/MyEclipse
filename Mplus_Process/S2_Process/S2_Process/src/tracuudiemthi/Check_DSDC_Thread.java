...
public class Check_DSDC_Thread extends Thread{
		...
		while(Sender.processData){
			...
			if(!"0".equalsIgnoreCase(somon)) {
				...
				for(int i =0; i< mon_da.length; i++) {
					if(!isExistMaTruong(mon_da[i])) {
						insertMatruong_DiemChuan(mon_da[i]);
					}
				}
			}
	...
	private boolean isExistMaTruong(String matruong) {
		...
		try {
			...
			String query1 = "select * from  matruong_diemchuan where matruong = '"+ matruong+ "'";
	...
	private int insertMatruong_DiemChuan(String matruong) {
		...
		if (connection != null) {
			try {
				String strQuery = "INSERT INTO matruong_diemchuan (matruong)"
						+ " VALUES(?)";
