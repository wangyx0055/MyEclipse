...
public class Check_DapAn_Thread extends Thread{
	...
	public void run(){
		...
		while(Sender.processData){
			...
			String call = url + "gameID=20&type=21";
			String xml = "";
			try {
				xml = Http_post(call);
			} catch (IOException e) {
			...
			if(!"0".equalsIgnoreCase(somon)) {
				dapan = getValue(xml, "mamon");
				String[] mon_da = dapan.split(",");
				for(int i =0; i< mon_da.length; i++) {
					if(!isExistMaMon(mon_da[i])) {
						insertDapan_Thamkhao(mon_da[i]);
					}
				}
			}
	...
	//Check xem mon da co trong table chua
	private boolean isExistDapAnChinhThuc(String mamon) {
		...
		try {
			...
			// TODO: handle exception
			Util.logger.info("isExistDapAnChinhThuc Failed" + e.getMessage());
		} finally {
			dbpool.cleanup(connection);
	...
	private int insertDapan_Thamkhao(String mamon) {
		...
		if (connection != null) {
			try {
				String strQuery = "INSERT INTO dapan_thamkhao (mamon)"
						+ " VALUES(?)";
