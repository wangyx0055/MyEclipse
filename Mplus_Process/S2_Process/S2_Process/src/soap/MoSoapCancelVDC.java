...
public class MoSoapCancelVDC extends QuestionManager {
	String className = "MoSoapCancelVDC  ";
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			String url = Constants._prop.getProperty("mo.soap.VDC.url");
			String sUserName = Constants._prop
					.getProperty("mo.soap.VDC.username");
			String sPassword = Constants._prop
					.getProperty("mo.soap.VDC.password");
			...
			if (iReturn == 1) {
				...
				MTPushObject mtPushObject = dbSelect.getMTPushByUserId(
						msgObject.getUserid(), tableName);
				if (mtPushObject != null) {
					DBInsert dbInsert = new DBInsert();
					Util.logger.info("SoapMOCancel ## " + "insert into "
							+ tableNameCancel + ", userid = "
							+ msgObject.getUserid());
					dbInsert.insertMTPush(mtPushObject, tableNameCancel,
							poolName);

					Util.logger
							.info(className + "@ " + "delete from " + tableName
