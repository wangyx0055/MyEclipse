...
import com.tekciz.webservice.SendMoToVascProxy;
...
public class MoSoapCancelTekzic extends QuestionManager {
	String className = "MoSoapCancelTekzic  ";
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			String url = Constants._prop.getProperty("mo.soap.TEKZIC.url");
			String sUserName = Constants._prop
					.getProperty("mo.soap.TEKZIC.username");
			String sPassword = Constants._prop
					.getProperty("mo.soap.TEKZIC.password");
			...
			if (iReturn == 1 || iReturn == 0 || iReturn == 2) {
				 ...
				 * 
				 * MTPushObject mtPushObject = dbSelect.getMTPushByUserId(
				 * msgObject.getUserid(), tableName); if (mtPushObject != null)
				 * { DBInsert dbInsert = new DBInsert();
				 * Util.logger.info("SoapMOCancel ## " + "insert into " +
				 * tableNameCancel + ", userid = " + msgObject.getUserid());
				 * 
				 * // dbInsert.insertMTPush(mtPushObject, //
				 * tableNameCancel,poolName);
				 * 
				 * Util.logger .info(className + "@ " + "delete from " +
				 * tableName + ", userid = " + msgObject.getUserid()); DBDelete
