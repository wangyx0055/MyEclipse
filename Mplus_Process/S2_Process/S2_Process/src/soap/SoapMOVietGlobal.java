 ...
 * ****/
public class SoapMOVietGlobal extends QuestionManager {

	public String SERVICE_THOITIET = "VIETGLOBAL";
	@Override
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {			
			...
			if (msgObject.getObjtype() == 0) {
				...
				Util.logger.info("Command_Code gui sang VietGlobal :" + commandcode);
				result = msgObject.sendMessageMO(msgObject.getUsertext(),
						"VIETGLOBAL", commandcode);
				

			} else {
				Util.logger.info("VIETGLOBAL @Operator: " + msgObject.getMobileoperator());
				result = msgObject.sendMessageMT(msgObject.getUsertext(),
						"VIETGLOBAL", keyword.getKeyword());
			}
			...
			if (result.equalsIgnoreCase("1") || result.equalsIgnoreCase("0")) {
				//ok
				Util.logger.info("@SoapMOVietGlobal@Push mo queue sang icom thanh cong!@user_id=" + msgObject.getUserid() 
				...
				msgObject.setTTimes(DateProc.createTimestamp());
					...
					DBUtil.Alert("SoapMOVietGlobal", "Sendfailed", "warn", "Sendfailed:"
							+ msgObject.getUserid() + "@"
							+ msgObject.getMsg_id(), "");
