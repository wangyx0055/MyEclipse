...
import MPlusServiceMobiTech.MPlusPortProxy;
...
public class SoapMoMobiTech  extends QuestionManager{
	String className = "SoapMoMobiTech ";
	@Override
	protected Collection getMessages(Properties prop, MsgObject msgObject,
			Keyword keyword, Hashtable services) throws Exception {
		
		try{
			MPlusPortProxy proxy = new MPlusPortProxy();
			String url = Constants._prop.getProperty("mo.soap.MobiTech.url");
			String sUserName = Constants._prop.getProperty("mo.soap.MobiTech.username");
			String sPassword = Constants._prop.getProperty("mo.soap.MobiTech.password");
			...
			Util.logger.info(className + " fw mo.@info :\t@sUser:"
		 	...
		 	int iReturn = proxy.moReceiver(sUserName, sPassword, sMsisdn,
		 			sServiceNumber, sServiceCode, sInfo, iRequestId+"", sOperator, iChannel_Type); 
		 	//(sUserName, sPassword, sMsisdn, sServiceNumber, sServiceCode, sInfo, iRequestId, sOperator, iChannel_Type);
			
		 	Util.logger.info(className +" @return code:"+iReturn+"\t@user:"+sMsisdn);
		 	
		 	if(iReturn ==1){
		 		Util.logger.info(className +" @Call class DBUtil.RegisterServices for register \t@user:"+sMsisdn);
		 		
		 		DBUtil.RegisterServices(msgObject, keyword, Constants.TYPE_OF_SERVICE_TEXTBASE,services);
		 	}else if(iReturn ==2 || iReturn ==3 || iReturn ==4 ||iReturn ==5 || iReturn ==6 || iReturn ==255){
