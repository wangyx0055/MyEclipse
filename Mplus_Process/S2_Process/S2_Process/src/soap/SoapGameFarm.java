...
import vms.to.gamefarm.moqueue.MOQueueSenderGameFarm;
 
 

public class SoapGameFarm extends QuestionManager {
	@Override
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			if (msgObject.getObjtype() == 0) {
				 ...
				 * Dich vu game farm
				 * **/
				result= MOQueueSenderGameFarm.insertMOQueueGameFarm(msgObject) +"";
			} else {
			...
			if (result.equals("1")) {
				/*****
				 * 2010-11-07: PhuongDT
				 * Sau khi push sang icom thanh cong, thuc hien dang huy ben VMS
				 * ***/
				Util.logger.info("@SoapGameFarm@Push mo queue sang game farm thanh cong! \tuser_id:" + msgObject.getUserid() + "\tkeyword:" +msgObject.getKeyword() + "\trequest_id:" + msgObject.getRequestid());
				return null;
			}
			int retriesnum = msgObject.getRetries_num();

			Util.logger.info("SoapGameFarm@"
			...
			if (retriesnum >= Constants.MAX_RETRIES) {
				...
				DBUtil.Alert("SoapGameFarm", "Sendfailed", "warn", "Sendfailed:"
