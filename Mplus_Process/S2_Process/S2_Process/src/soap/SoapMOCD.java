...
public class SoapMOCD extends QuestionManager {
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			Util.logger.info("SoapMOCD ; user_id = " + msgObject.getUserid() 
			...
			if(result.equals("1")){
				//huy TC, dk EU
				Util.logger.info("SoapMOCD : send MO success! \t@userid:"+msgObject.getUserid());
				String tableName = "customer_cancel";
				if(isexistCustomer(msgObject.getUserid(),tableName)){
					...
					//reset scoure
					//ResetScore(msgObject.getUserid());
				}else{
	...
	private static BigDecimal add2MOSendFailed(MsgObject msgObject) {
		...
		Util.logger.info("add2MOSendFailed:"+sSQLInsert);
	...
	public static void MoveCustomerCancel2Customer(String user_id) {
		...
		String sqlUpdateMlistUser = "INSERT INTO customer(msisdn,LastQuestionId,SubscriptionType," +
				...
				+",TotalScore,TotalMOByDay,Reg_Again)"
				 +"SELECT msisdn,LastQuestionId,SubscriptionType,TotalRepeatFail,CreateDate"
				+",CustomerLevel,NOW(),pid,QuestionSended,CurrentScore,DiemChuaSinhMDT,DiemDaSinhMDT,0"
				+",0,TotalMOByDay,Reg_Again +1 "
	...
	public static int insertCustomer(String msisdn,String tableName)
	{
		...
		try
		{
			...
			String sqlInsert="insert into " +tableName+" (MSISDN,Pid,LastUpdate" +
					") values ('"+msisdn+"',"+getPid(msisdn)+",'"+time+"')"; 
