...
public class SoapBDAnh extends QuestionManager {
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			if (msgObject.getObjtype() == 0) {
				...
				Util.logger.info("Usertext send to Beeline: "
						+ msgObject.getUsertext());
				Util.logger.info("Operator: " + msgObject.getMobileoperator());

				if ("Operator".equalsIgnoreCase(msgObject.getMobileoperator())) {
					msgObject.setMobileoperator("BEELINE");
				}
				Util.logger.info("Service_ID: " + msgObject.getServiceid());
				Util.logger.info("Keyword Msg: " + msgObject.getKeyword());
				
				
				// Bong da Anh
				msgObject.setServiceid("1692");
				msgObject.setKeyword("BD ANH");
