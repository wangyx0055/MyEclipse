...
public class MusicWap extends QuestionManager {
		...
		if (sTokens.length < 2) {
			...
			if (!INVALID_CODE.equalsIgnoreCase(VALID_CODE)) {
				...
				if (saverequest(phone, VALID_CODE, type, GID, DBCONTENT)) {
					...
					if (sTokens.length > 3) {
						giff_userid = ValidISDNNew(sTokens[3]);
					...
					if (giff == true) {
						...
						MsgObject msgObj = new MsgObject(1, msgObject
								...
								.getKeyword(), "Sodt " + msgObject.getUserid()
								+ " tang ban " + code + ":" + SERVER + "/?p="
								+ phone + "&c=" + code + "&f=" + type + "&g="
								+ GID, msgObject.getRequestid(), DateProc
								.createTimestamp(), giff_telco, 0, wappush);
						...
						msgObject.setUsertext(code + ":" + SERVER + "/?p="
								+ phone + "&c=" + code + "&f=" + type + "&g="
								+ GID);
						msgObject.setContenttype(wappush);
						msgObject.setMsgtype(1);
						messages.add(new MsgObject(msgObject));
					}
