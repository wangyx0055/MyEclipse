 ...
 * @version 2.0
 */
public class ExecuteInsertListSendVMS extends Thread {
	...
	class InsertListSendTask extends TimerTask {
		...
		public void run() {
			...
			String sqlSelectPacket = "SELECT id, services, hours, options, timesendcharge, packet_or_mt FROM "
					...
					+ sDay
					+ "%' or upper(dayofweek)='X' ) and run_insert_list_send = 0 and result = 0"
					+ " and packet_or_mt = " + Constants.SERVICE_PAGKET;

			Vector<Object> vtServicePacket = null;
			System.out.println("SQL SELECT = " + sqlSelectPacket);
			try {
	...
	private int insertToListSend(MlistInfo mlistInfo, String timeDelivery){
		...
		try {
			...
			stmt.setInt(16, Constants.PACKET_VMS);
	...
	private static boolean updateRunInsertService(String serviceId) {
		...
		try {
			...
			if (connection == null) {
				Util.logger
						.error("VMS@ExecuteInsertListSendVMS@updateResultServices@connection is null.");
				return false;
			}
			...
			Util.logger
					.info("VMS@ExecuteInsertListSendVMS@updateResultServices@SQL UPDATE: "
							+ sSQL);
			statement = connection.prepareStatement(sSQL);
			if (statement.execute()) {
				Util.logger.error("VMS@ExecuteInsertListSendVMS@updateResultServices@"
			...
			Util.logger
					.error("VMS@ExecuteInsertListSendVMS@updateResultServices@: Error:"
							+ e.toString());
			return false;
		} catch (Exception e) {
			Util.logger
