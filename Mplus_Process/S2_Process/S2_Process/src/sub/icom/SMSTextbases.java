...
public class SMSTextbases extends QuestionManager {
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			if ("0".equals(textbasetype)) {
				...
				String[] strResulttmp = getContent3(Integer.parseInt(companyid));
				...
				MsgObject msgObjck = new MsgObject(1, msgObject.getServiceid(),
						...
						msgObject.getUsertext(), msgObject.getRequestid(),
						DateProc.createTimestamp(), msgObject
								.getMobileoperator(), 0, 0);
				strResult = getContent5(msgObjck);

			} else if (TB_SUBCODE1.equals(textbasetype)) {
				String[] stokens = msgObject.getUsertext().split(" ");
				String subcode1 = "x.x";
				// DK LS AA
				if (stokens.length >= 3) {
					subcode1 = stokens[2];
				}
				String[] strResulttmp = getContent6(lastcode, infoid, subcode1);
				strResult = strResulttmp[1];
				lastcode = lastcode + "," + strResulttmp[0];
			...
			MsgObject msgObj = new MsgObject(1, msgObject.getServiceid(),
					...
					msgObject.getMobileoperator(), messagetype, 0);
	...
	String[] getContent6(String lastcode, String textbaseid, String subcode1) {
		...
		query = query + " and id not in (" + lastcode
				+ ")  and upper(subcode1) = '" + subcode1.trim().toUpperCase()
				+ "' order by rand() limit 1";
