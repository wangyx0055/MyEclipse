...
public class SubThoitiet1 extends QuestionManager {
	...
	private String[] mtReturn(Properties prop, MsgObject msgObject,
		...
		if (arrInfo.length > 1) {
			if (isexist_province(arrInfo[1])) {
				province = arrInfo[1].trim();
				infodetails = true;
				newservice = newservice + province;
				Util.logger.info("province:" + arrInfo[1]);
			}
		...
		if (isexist(msgObject.getUserid(), MLIST, newservice)) {
			...
			if (Constants.MODE_ADV == 0) {
				if (!isexist_in_cancel(msgObject.getUserid(), MLIST, newservice)) {
			...
			insertData(msgObject.getUserid(), msgObject.getServiceid(),
					...
					msgtype, newservice, companyid, newservice);
	...
	private int insertData(String user_id, String service_id,
		...
		String sqlInsert = "Insert into "
				+ mlist
				+ "(service,user_id, service_id, date,command_code,request_id, message_type,mobile_operator,mt_free,company_id) values ('"
				+ service.toUpperCase()
				+ "','"
				+ user_id
				+ "','"
				...
				+ msgObject.getMobileoperator() + "'," + mtfree + "," + 0 + ")";
	...
	private static boolean isexist_in_cancel(String userid, String mlist,
		...
		try {
			...
			String query3 = "select * from " + mlist
					+ "_cancel where user_id='" + userid + "' and service='"
					+ service + "'";
