...
public class SubCK extends QuestionManager {
	...
	private String[] mtReturn(Properties prop, MsgObject msgObject,
		...
		String infoid = getStringfromHashMap(_option, "infoid", "CK");
		...
		if ("CKT".equalsIgnoreCase(infoid)) {
			syntax = "CK";
		}
		...
		String sservice = keyword.getService_ss_id();
		...
		if (isexist(msgObject.getUserid(), MLIST, sservice)) {
			...
			insertData(msgObject.getUserid(), msgObject.getServiceid(),
					...
					msgtype, keyword.getService_ss_id(), syntax, sservice);
			// mtReturn[0] = keyword.getSubMsg();
	...
	private int insertData(String user_id, String service_id,
		...
		String sqlInsert = "Insert into "
				+ mlist
				+ "(service,user_id, service_id, date,command_code,request_id, message_type,mobile_operator,mt_free,options) values ('"
				+ service.toUpperCase()
				+ "','"
				+ user_id
				+ "','"
