...
public class SubMatch extends QuestionManager
{
			...
			Keyword keyword, int msgtype) throws Exception
	{
		...
		else if ("-1".equalsIgnoreCase(result[0]))
		{
			mtReturn[0] = fails;
			return mtReturn;
		}
		...
		else
		{
			...
			insertData(msgObject.getUserid(), msgObject.getServiceid(),
					...
					result[1], keyword.getAmount(), msgObject.getContentId(),
