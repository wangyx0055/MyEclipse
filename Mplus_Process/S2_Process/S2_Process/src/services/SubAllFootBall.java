...
public class SubAllFootBall extends QuestionManager
{
	protected Collection getMessages(Properties prop, MsgObject msgObject,
			Keyword keyword, Hashtable services) throws Exception
	{
		Collection messages = new ArrayList();
		int msg1mt = Integer.parseInt(Constants.MT_CHARGING);
			...
			Keyword keyword, int msgtype) throws Exception
	{
		...
		String code = "x";
		if (isexist(msgObject.getUserid(), MLIST, code))
		{
			// Da ton tai trong danh sach khach hang
		...
		else
		{
			...
			{
				if (!isexist_in_cancel(msgObject.getUserid(), MLIST, sservice,
						code))
				{
			...
			// Ghi thong tin khach hang vao database
			insertData(msgObject.getUserid(), msgObject.getServiceid(),
					msgObject.getServiceid(), MLIST, msgObject, mtfree,
					msgtype, keyword.getService_ss_id(), companyid, sservice,
					code, keyword.getAmount(), msgObject.getContentId(),msgObject.getChannelType());
	...
	public String[] getCode(String dbgw, String football_match)
	{
		...
		try
		{
			...
			String sqlSelect = "SELECT active, Match_ID FROM worldcup_match WHERE upper(Match_ID) = '"
	...
	/**
	 * <p>
	 * Title: Insert thông tin đăng ký vào table mlist
	 * </p>
	 * <p>
	 * Description:
	 * </p>
	...
	private int insertData(String user_id, String service_id,
			String command_code, String mlist, MsgObject msgObject,
			String mtfree, int msgtype, String Service_ss_id, String companyid,
			String service, String horoscope, long amount, int content_id, int channelType)
	{
		int ireturn = 1;
		Connection connection = null;
		DBPool dbpool = new DBPool();
		...
		String sqlInsert = "Insert into "
				+ mlist
				+ "(service,user_id, service_id, date,command_code,request_id, message_type,mobile_operator,mt_free,company_id,options, amount, content_id, channel_type) values ('"
				+ service.toUpperCase()
				+ "','"
				+ user_id
				+ "','"
