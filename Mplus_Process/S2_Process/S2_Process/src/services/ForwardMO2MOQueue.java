...
public class ForwardMO2MOQueue extends QuestionManager {

	@Override
	protected Collection getMessages(Properties prop, MsgObject msgObject,
			Keyword keyword, Hashtable services) throws Exception {

		Util.logger.info("ForwardMO2MOQueuestart! user_text:"
		...
		if (insertMoQueue("mo_queue", msgObject) != -1) {
			Util.logger
					.info("insertMoQueueDB ok user:" + msgObject.getUserid());
		} else {
			Util.logger.error("insertMoQueueDBE false user:"
					+ msgObject.getUserid());
		}
	...
	private static int insertMoQueue(String tableName, MsgObject msg) {
		...
		try {
			cnn = dbpool.getConnection("countdown");
