...
public class Send2ck extends QuestionManager {
	...
	protected Collection getMessages(Properties prop, MsgObject msgObject,
		...
		try {
			...
			String cp = getString(_option, "cp", "CK").toUpperCase();

			String keyword_incom = getString(_option, "key", keyword
					.getKeyword());

			String result = "";

			// result = insertMO2lottery(msgObject,new_serviceid);
			String user_text = msgObject.getUsertext().replace(" ", "");
			Util.logger.info("Noi DUNG GUI INCOM");
			Util.logger.info("INFO: " + user_text);
			Util.logger.info("Keyword: " + keyword_incom);
			msgObject.setUsertext(user_text);
			result = msgObject.sendMessageMO(user_text, cp, keyword_incom);
			
			if (result.equals("1")) {
				msgObject.setMobileoperator("BEELINE");
