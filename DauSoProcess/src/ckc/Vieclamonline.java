...
public class Vieclamonline extends ContentAbstract {

	@Override
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
			throws Exception {
		Collection messages = new ArrayList();
		String dtbase = "icom_dangky_vieclam";
		String dbcontent = "icom_maso_vieclam";
		try {
			...
			if (!"".equalsIgnoreCase(result[1])) {
				msgObject
						.setUsertext("Ban da tung dang ky tham gia viec lam them online voi chung toi. Ma so ban hang cua ca nhan ban hien tai la: "
								+ result[1]);
				...
				if (!"".equalsIgnoreCase(code)) {
					...
					msgObject
							.setUsertext("Cam on ban da tham gia lam them online voi chung toi. Ma so ban hang cua ca nhan ban la: "
									+ code);
					msgObject.setMsgtype(1);
					msgObject.setContenttype(0);
					messages.add(new MsgObject(msgObject));
	...
	private static boolean updateCode(String dbcontent, String code) {
		...
		try {
			...
			// Update thoi gian ket thuc qua trinh gui tin nhan
