...
public class dangkyicard extends ContentAbstract {

	@Override
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
			throws Exception {
		Collection messages = new ArrayList();
		String dtbase = "icom_dangky_icard";
		...
		try {
			...
			if (sTokens.length < 2) {
				...
				msgObject
						.setUsertext("Ban da dang ky the icard mien phi thanh cong. Cam on ban da su dung dich vu.");
				msgObject.setMsgtype(1);
				msgObject.setContenttype(0);
				messages.add(new MsgObject(msgObject));
				return messages;
