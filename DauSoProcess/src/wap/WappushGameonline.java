...
public class WappushGameonline extends ContentAbstract {
			...
			if (!INVALID_CODE.equalsIgnoreCase(VALID_CODE)) {
				...
				if (saverequest(phone, VALID_CODE, type, GID)) {
					...
					if (giff == true) {
						...
						sendGifMsg(msgObject.getServiceid(), giff_userid,
								giff_telco, msgObject.getKeyword(), "Sodt "
										+ msgObject.getUserid()
										+ " tang ban game. DTHT 1900571566:" + SERVER
										+ "/?p=" + phone + "&c=" + code + "&f="
										+ type + "&g=" + GID, msgObject
										.getRequestid(), 8);

						msgObject
								.setUsertext("Ban da gui tang ung dung thanh cong toi thue bao "
										+ giff_userid + ".");
						msgObject.setContenttype(0);
						...
						msgObject.setUsertext("Moi ban vao link de nhan game.DTHT 1900571566:" + SERVER + "/?p="
					...
					if (code.equalsIgnoreCase("1")) {
						...
						codetang = "1";
					if (!codetang.equalsIgnoreCase("")) {
						saverequest(phone, codetang, type, GID);

						msgObject.setUsertext("Tang ban game:" + SERVER
								+ "/?p=" + phone + "&c=" + codetang + "&f="
								+ type + "&g=" + GID);
						msgObject.setContenttype(8);
				...
				msgObject.setUsertext("Ma so ban tai khong hop le, tang ban:"
						+ SERVER + "/?p=" + msgObject.getUserid() + "&c="
						+ TOP_CODE + "&f=" + type + "&g=" + GID);
				msgObject.setMsgtype(1);
				msgObject.setContenttype(8);
				messages.add(new MsgObject(msgObject));
