...
public class PicturesWapQM extends ContentAbstract {
	int GID = 0;
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length == 1) {
				msgObject
						.setUsertext("[ICom] Ma so ban tai khong hop le. Tang ban TOP hinh:"
								+ URL_INV);
				...
				if (validcode(sTokens[1], GID)) {
					String phone = msgObject.getUserid();
					String code = sTokens[1];
					String type = "image";

					if (saverequest(phone, sTokens[1], type, GID)) {
						...
						if (giff == true) {
							...
							sendGifMsg(msgObject.getServiceid(), giff_userid,
									...
									msgObject.getUserid() + " tang ban " + code
											+ ":" + SERVER + "/?p=" + phone
											+ "&c=" + code + "&f=" + type
											+ "&g=" + GID, msgObject
											.getRequestid(), 8);
					...
					msgObject
							.setUsertext("Ma so ban tai khong hop le. Tang ban TOP hinh:"
									+ URL_INV);
	...
	private boolean validcode(String code, int gid) {
		...
		try {
			...
			String query1 = "select code,filetype from icom_wap.dbo.upload  where upper(code)='"
					+ code.toUpperCase()
					+ "' and upper(filetype)='IMAGE' and cgroup=" + gid;
