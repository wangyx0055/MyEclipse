 ...
 * @version 1.0
 */
public class DKnhaccho extends ContentAbstract {
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				if (!infoid.equalsIgnoreCase("viettel")) {
					// Dich vu khong ho tro mang khac ngoai vietttel
					...
					if ("viettel".equalsIgnoreCase(infoid)) {
						...
						try {
							...
							if (result2.equalsIgnoreCase("0")) {
								mt1 = "Ban dang ky su dung dich vu nhac cho Imuzic thanh cong. DTHT 1900571566";
								msgObject.setMsgtype(1);
								
							} else  {
								mt1 = "Ban dang ky khong thanh cong xin vui long thu lai hoac goi den tong dai 1900571566 de duoc ho tro";
								msgObject.setMsgtype(2);
				...
				msgObject
						.setUsertext("Cam on da su dung dich vu, ban nhan tin sai cu phap. DTHT 1900571566");
