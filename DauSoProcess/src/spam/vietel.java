...
public class vietel extends SpamQM {
	...
	protected String [] checkspam(MsgObject msgObject) throws Exception {
		...
		sRet[1] = "";
		try {
			return checkspamindb(msgObject.getUserid(),msgObject.getMobileoperator());
		} catch (Exception e) {
	...
	private boolean checkspamindbbycount(String user_id, String operator) {
		String tablename = "sms_receive_day"
				+ new SimpleDateFormat("yyyyMM").format(new Date());

		String sqlselect = "";
		String sql5min = "select count(*)-3,5  from sms_receive_day_" + operator.toLowerCase() +" where user_id='"
				+ user_id
				+ "' and receive_date > TIMESTAMPADD (minute,-5,current_timestamp )";
		String sql10min = "select count(*)-5,10  from sms_receive_day_" + operator.toLowerCase() +" where user_id='"
				+ user_id
				+ "' and receive_date > TIMESTAMPADD (minute,-10,current_timestamp )";
		String sql60min = "select count(*)-30,60  from sms_receive_day_" + operator.toLowerCase() +" where user_id='"
				+ user_id
				+ "' and receive_date > TIMESTAMPADD (minute,-60,current_timestamp )";
		String sqlallday = "select count(*)-300,1440  from sms_receive_day_" + operator.toLowerCase() +" where user_id='"
				+ user_id + "' ";
		Connection connection = null;
