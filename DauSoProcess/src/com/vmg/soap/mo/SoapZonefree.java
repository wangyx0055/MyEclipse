...
public class SoapZonefree extends ContentAbstract {
...
String result = "";
		while (iRetries > 0) {
			try {
				String url = Constants._prop.getProperty("mt.soap." + keyword.getOptions() + ".url");
			
				if (url == null)
					throw new Exception("In the profile is missing mt.soap." +keyword.getOptions()
							+ ".url");
				...
				//result = sendXMLZonefree.updateStatus( url, msgid, from, text, shortcode,msgtype);
