...
public class WappushRing extends ContentAbstract {	
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		//	id = id + 1;
			...
			if (sTokens.length == 1) {
				...
				int cateid = GateCateid(sTokens[1]);
				...
				String namecode=GateCateName(sTokens[1]);
				...
				String link = SoapMOMT.SendXML(4,
						msgObject.getUserid(), code+"", 3, SoapMOMT.GetPrice(msgObject.getServiceid()), now());
				Util.logger.info("link:"+link);

				if (link.startsWith("0")) {
					msgObject.setUsertext(namecode+":"+link.substring(2));
					//if(SoapMOMT.SendMT(msgObject))
					//	Util.logger.info("Post MT OK__");
					//else
					//	Util.logger.info("Ko Post Duoc___");
				...
				{
					msgObject.setUsertext("Loi he thong");
					msgObject.setContenttype(0);
					msgObject.setMsgtype(1);					
	...
	public static int getCode(int CateID, String lastcode) {
		...
		Connection connection = null;
		DBPool dbpool = new DBPool();
		String query = "SELECT Top 1 [RingtoneID] FROM [IComStore].[dbo].[Ringtone]  where CateID="
				+ CateID
	...
	public static int GateCateid(String command_code) {
		int cateid = 0;
		if (command_code.equalsIgnoreCase("VP")) {
			cateid = 421;
		}
		if (command_code.equalsIgnoreCase("QT")) {
			cateid = 423;
		}
		if (command_code.equalsIgnoreCase("RK")) {
			cateid = 448;
		}
		if (command_code.equalsIgnoreCase("HAN")) {
			cateid = 422;
		}
		if (command_code.equalsIgnoreCase("DD")) {
			cateid = 425;
		}
		if (command_code.equalsIgnoreCase("DOC")) {
			cateid = 434;
		}
		return cateid;
	}
