...
public class SubHoangDao extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		String[] cung = { "BACHDUONG", "KIMNGUU", "SONGTU", "CUGIAI", "SUTU",
				"XUNU", "THIENBINH", "HOCAP", "NHANMA", "MAKET", "BAOBINH","SONGNGU" };
		...
		Util.logger.info("temp1____" + temp1);
			...
			String shoroscope = arrInfo[1];
			boolean kt=false;
			for (int i=0;i<cung.length;i++)
			{
				if (shoroscope.equalsIgnoreCase(cung[i]))
					kt=true;
			}
			if (kt=false)
			{
				msgObject
				.setUsertext("Sai ten cung hoang dao. Soan tin "
			...
			if (!"".equalsIgnoreCase(content)) {
				...
				for (int i = 0; i < mess.length; i++) {
					if (!"".equalsIgnoreCase(mess[i])) {
						...
						// log send MT		
						String temp3 = SoapMOMT.SendMT(msgObject);
