...
public class SubTruyencuoi extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		MLIST = getStringfromHashMap(_option, "mlist", "mlist_truyencuoi_moststep");
		Util.logger.info("mlist: " + MLIST);
		String mtfree = getStringfromHashMap(_option, "mtfree", "7");
		int mtfreeInt = 0;
		mtfreeInt = Integer.parseInt(mtfree);
		String duration = getStringfromHashMap(_option, "duration", "7");
		String sservice = getStringfromHashMap(_option, "service", "TRUYENCUOI");
		String companyid = getStringfromHashMap(_option, "companyid", "74");
	...
	//	String DBCONTENT ="gateway";
		...
		String success = getStringfromHashMap(
				_option,
				"success",
				"Ban da dang ky thanh cong dich vu Truyen cuoi.Noi dung se duoc gui den ban hang ngay.Cam on da su dung dich vu.DTHT 1900571566.");
		...
		if (isexist(msgObject.getUserid(), MLIST, sservice, bongda)) {
			...
			insertData(msgObject.getUserid(), msgObject.getServiceid(),
					...
					msgObject.getKeyword(), companyid, sservice, bongda,duration);
	...
	private String getInfo() {
		...
		try {
			...
			connection = dbpool.getConnection("textbase");

			String query = "SELECT content FROM icom_textbase_data  where gameid='truyencuoi'  Order by rand() limit 1";
	...
	private int insertData(String user_id, String service_id,
			...
			String service, String bongda,String duration) {
		int ireturn = 1;
		Connection connection = null;
		DBPool dbpool = new DBPool();
		...
		String sqlInsert = "Insert into "
				+ mlist
				+ "(service,user_id, service_id, date,command_code,request_id, message_type,mobile_operator,mt_free,company_id,options,duration) values ('"
				+ service.toUpperCase()
				+ "','"
				+ user_id
				+ "','"
