...
public class DailyTruyencuoi extends DeliveryManager {
	...
	protected Collection getMessages(String ssid, String option,
		...
		try {
			...
			String DBCONTENT = "";
			String INFO_ID = "x";
			String MLIST = "mlist_truyencuoi_moststep";
			String CLASSNAME = "DailyTruyencuoi";
			...
			String currDate = new SimpleDateFormat("dd/MM/yyyy")
					...
					arrInfo = getInfo(DBCONTENT, currDate);
				//if (arrInfo[i] == null) {
					//return null;
				//}
			...
			String sqlSelect = "select id, user_id, service_id, last_code,command_code," +
			"request_id, message_type,mobile_operator,mt_count,mt_free,duration,"+
		...
		try {
			...
			for (int i = 0; i < content.length; i++) {
				...
				if (!"".equalsIgnoreCase(content[i])) {
					...
					msgObject.setUsertext( content[i]);
