		...
		try {
			...
			if (sTokens.length == 2) {
				...
				if (contentid.equalsIgnoreCase("")) {
					...
					if (usergif.equalsIgnoreCase("")) {

						Util.logger.info("contentid=" + contentid);

						mt1 = "Ban cai dat nhac cho chua thanh cong vui long soan DK gui 1221";
						try {
							...
							if (result.equalsIgnoreCase("0")) {
								mt1 = "Ban cai dat nhac cho thanh cong.Cam on da su dung dich vu.DTHT 1900571566";
							} else {
						...
						mt1 = "Ban gui tang chua thanh cong do chua dang ky nhac cho chua thanh cong vui long soan DK gui 1221";
						try {
							String result = sendXMLRING.SendXML(urlring,
									"ConfirmValidCode", userid, sTokens[1], "",
									now());
