 ...
 * 
 * <pre>
 *Dich vu cho phep tai ve danh sach ten game, ma so game
 * </pre>
 * 
 * @author Haptt
 * @version 1.0
 */
public class Gamechude extends ContentAbstract {

	/* First String */
		
	private static String dbcontent = "icom_dangky_gamechude";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			// Send MT 
				...
				if (timesend >= 1) {
					// Gui lan thu 2
					lastid = getGame(userid, dbcontent, 0);
				} else {
				...
				// MT2 
				String listgame = findListofGame(dtbase, userid, infoid,
						lastid, chude);
				...
				String sendtoClient = "Ngoai nhung game ban vua nhan duoc,van con rat nhieu game khac tren "+numberid+".De tiep tuc nhan duoc ma so game."
						+ last_msg;
	...
	private static String getGame(String userid, String dtbase, int subcode) {
		...
		try {
			...
			String query = "SELECT magame FROM " + dtbase
	...
	private static boolean updateListGame(String userid, String lastid,
		...
		try {
			...
			String sqlUpdate = "UPDATE " + dtbase + " SET magame = '"
	...
	/* Tim list bai hat */
	private static String findListofGame(String dtbase, String userid,
		...
		try {
			...
			String sqlSelect = "SELECT tengame,magame FROM icom_game WHERE chude=' "+ chude+"' ";
			Util.logger.info("last id : " + lastid);
			
			if (!"".equalsIgnoreCase(lastid)) {
				Util.logger.info(""+!"".equalsIgnoreCase(lastid));
					
				sqlSelect = sqlSelect + " AND magame not in (" + lastid + ") ";
			}
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					if (result.length() > 160) {
						...
						updateListGame(userid, lastid, dbcontent, chude);
						return result;
					} else {
						if ("".equalsIgnoreCase(lastid)) {
