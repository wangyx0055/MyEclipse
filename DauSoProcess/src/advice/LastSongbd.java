...
public class LastSongbd extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String stime = getString(_option, "time", " 13h thu 2 hang tuan.");
			...
			Hashtable lastSong = Threadlastsongbd.lastestsong.LAST_SONG;
			...
			if (Threadlastsongbd.lastestsong.isProcess == 0) {
				msgObject
						.setUsertext("Thoi gian du doan da het.Hay lang nghe chuong trinh va tham gia du doan bai hat vao "+stime+".DTHT:1900571566");
				msgObject.setContenttype(0);
				msgObject.setMsgtype(1);
				messages.add(new MsgObject(msgObject));
				return messages;
			} else if (Threadlastsongbd.lastestsong.isProcess == 1) {
			...
			} else if (Threadlastsongbd.lastestsong.isProcess == 2) {
				msgObject
				...
				messages.add(new MsgObject(msgObject));
				return messages;
			} else if (Threadlastsongbd.lastestsong.isProcess == 3) {
	...
	private static boolean saveClient(String session, String user_id,
		...
		try {
			...
			String sqlInsert = "INSERT INTO icom_lastsong_customerbd (session, user_id, service_id, mobile_operator, info, request_id, exactly, block, receive_time, cpid, keyword ) VALUES ('"
					+ session
					+ "','"
					+ user_id
					+ "','"
	...
	private static boolean saveAnswer(String session, String user_id,
		...
		try {
			...
			String sqlInsert = "INSERT INTO icom_lastsong_answerbd (session, user_id, service_id, mobile_operator, answer, request_id, correct, receive_date, cpid ) VALUES ('"
					+ session
					+ "','"
					+ user_id
					+ "','"
