...
public class QuatangamnhacHN extends ContentAbstract {
	@Override
		...
		try {
			...
			if (sTokens.length < 4) {
				String str = "Ban da nhan tin sai.Soan " + keywords
						+ " tenbaihat sodienthoainhan loinhan gui "
						+ service_id
				...
				giff_userid = sTokens[2];

				giff_userid = ValidISDNNew(giff_userid);
				if ("-".equalsIgnoreCase(giff_userid)) {
					String str = "Ban da nhan tin sai.Soan "
							+ keywords
							+ " tenbaihat sodienthoainhan loinhan gui "
							+ service_id
							+ " de tang bai hat cho nguoi than yeu.DTHT 1900571566";
				...
				sendGifMsg(
						...
						"Ban duoc sdt "
								+ userid
								+ " gui tang mon qua am nhac.Chuc ban vui ve.DTHT 1900571566",
						msgObject.getRequestid(), 0);
				String loinhan="";
				for (int k = 3; k < sTokens.length; k++) {
					loinhan = loinhan + sTokens[k] ;
				}
				saveClient(userid, sTokens[1], sTokens[2], loinhan, operator);
				updateTimes(userid,getTimes(userid));
				msgObject
	...
	private static boolean saveClient(String userid, String baihat,
			String giftuser, String loinhan, String operator) {
		...
		try {
			...
			String sqlInsert = "INSERT INTO quatangamnhachn( user_id,baihat,loinhan,gift_user,mobile_operator) VALUES ('"
					+ userid
					+ "','"
					+ baihat
					+ "','"
			...
			if (statement.execute()) {
				Util.logger.error("Insert into quatangamnhachn");
				return false;
			}
			return true;
	...
	public static int getTimes(String userid) {
		...
		try {
			...
			String query = "SELECT count(1) FROM quatangamnhacbd WHERE user_id='"+userid+"'";
		

			Vector result = DBUtil.getVectorTable(connection, query);
			for (int i = 0; i < result.size(); i++) {
				Vector item = (Vector) result.elementAt(i);
				tmpOperator = Integer.parseInt( item.elementAt(0).toString());
			}
	...
	private static boolean updateTimes(String userid,int times) {
		...
		try {
			...
			String sqlUpdate = "UPDATE quatangamnhacbd SET times = '"
					+ times + "' WHERE upper(user_id)='" + userid.toUpperCase()
					+ "' ";
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
				Util.logger.info("Update times " + userid
						+ " to dbcontent");
				return false;
			}
			return true;
