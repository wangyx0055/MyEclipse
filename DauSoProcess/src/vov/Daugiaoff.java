...
public class Daugiaoff extends ContentAbstract {
	...
	private String[] checkSession() {
		...
		try {
			...
			String query2 = "select * from daugia_phienoff where current_timestamp >"
			...
			if (statement.execute()) {
				...
				if (rs.next()) {
					...
					DBUtil
							.executeSQL(
									connection,
									"insert into daugia_phienoff(phien,begintime,endtime) select phien + 1,adddate(begintime,7),adddate(endtime,7)  from daugia_phienoff where phien="
											+ (phien - 1));
				}
			}
			dbpool.cleanup(rs, statement);

			String query5 = "select begintime,endtime,sanpham from daugia_phienoff where phien="
					+ phien;
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(1) from (select * from daugia_sieureoff where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugia_sieureoff where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_sieureoff (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			if (statement.executeUpdate() != 1) {
				Util.logger
						.info("daugia_sieureoff: Insert into daugia_sieureoff Failed");
			}
			String query1 = "select price from daugia_sieureoff where user_id='"
					+ userid + "'";
			statement = connection.prepareStatement(query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugia_sieureoff where phien="
							+ phienid;
					statement = connection.prepareStatement(query2);
					...
					String query3 = "select count(1) from daugia_sieureoff where price ="
				...
				String query4 = "select count(1) from daugia_sieureoff where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugia_winneroff where phien ="
					+ phien;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugia_sieureoff where phien ="
	...
	private static String[] getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT phien,begintime,endtime,sanpham FROM daugia_phienoff  where isprocess=1 ";
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winneroff (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.info("SieuRe: Insert in to daugia_winneroff Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winneroff  SET command_code ='"
	...
	private static boolean updateDuynhat(int phien, int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_phienoff  SET duynhat ='" + price
					+ " ' WHERE phien=" + phien;
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadauoff  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winneroff  WHERE phien="
					+ phien;
