...
public class Daugiav6 extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				msgObject.setUsertext("De biet khoang gia dang chiem uu the,soan: VG gui 8251.De biet muc gia ban muon da co ai dat chua,soan:VG gui 8551.DTHT:1900571566");
			...
			if (iNum == 0) {
				if (isEqual == 1) {
					...
					mtReturn = "Ban da dat gia "
							+ iPrice
							+ "000d.Day la muc gia thap nhat nhung khong phai la duy nhat.Tren muc gia nay con "
							+ num
							+ " chua ai dat, nhanh tay dat gia khac de co co hoi so huu "
							+ sSanpham;
				...
				if (isEqual == 1) {
					...
					mtReturn = "Muc ban tra "
							+ iPrice
							+ "000d.Day la duy nhat nhung khong phai la thap nhat.Duoi muc gia nay con "
							+ num
							+ " muc gia thap chua ai dat. Nhanh tay dat gia khac de tang co hoi so huu  "
							+ sSanpham;
				}
			}
		} else {
	...
	private String[] checkSession() {
		...
		try {
			...
			String query2 = "select * from daugia_phienv6 where current_timestamp >"
			...
			if (statement.execute()) {
				...
				if (rs.next()) {
					...
					DBUtil
							.executeSQL(
									connection,
									"insert into daugia_phienv6(phien,begintime,endtime) select phien + 1,adddate(begintime,7),adddate(endtime,7)  from daugia_phienv6 where phien="
											+ (phien - 1));
				}
			}
			dbpool.cleanup(rs, statement);

			String query5 = "select begintime,endtime,sanpham from daugia_phienv6 where phien="
					+ phien;
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(1) from (select * from daugia_sieurev6 where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugia_sieurev6 where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_sieurev6 (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			if (statement.executeUpdate() != 1) {
				Util.logger
						.info("daugia_sieurev6: Insert into daugia_sieurev6 Failed");
			}
			String query1 = "select price from daugia_sieurev6 where user_id='"
					+ userid + "'";
			statement = connection.prepareStatement(query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugia_sieurev6 where phien="
							+ phienid;
					statement = connection.prepareStatement(query2);
					...
					String query3 = "select count(1) from daugia_sieurev6 where price ="
				...
				String query4 = "select count(1) from daugia_sieurev6 where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugia_winnerv6 where phien ="
					+ phien;
	...
	private static String Countcanduoi(String price) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadauv6  where id>=1 and id<= "
				+ price;
	...
	private static String Countcantren(String price, String winprice) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadauv6  where id>=" + price
				+ " and id<= " + winprice;
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winnerv6 (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.info("SieuRe: Insert in to daugia_winnerv6 Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winnerv6  SET command_code ='"
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadauv6  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winnerv6  WHERE phien="
					+ phien;
