...
public class Daugiahn extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				msgObject.setUsertext("De biet khoang gia dang chiem uu the,soan: DG gui 8551.De biet muc gia ban muon da co ai dat chua,soan:DG gia gui 8551.DTHT:1900571566");
	...
	private static String Countcanduoi(String price) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadau  where id>=1 and id<= "
				+ price;
	...
	private static String Countcantren(String price, String winprice) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadau  where id>=" + price
				+ " and id<= " + winprice;
