...
public class Dautri extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			Hashtable numberDelay = Threaddautri.lastestnumber.SECRET_NUMBER;
			...
			if (Threaddautri.lastestnumber.isProcess == 0) {
				...
				messages.add(new MsgObject(msgObject));
				return messages;
			} else if ((Threaddautri.lastestnumber.isProcess == 1)) {
	...
	private boolean updateInfoCustomer(String database_customer,
		...
		try {
			...
			+ status + "=" + time
					...
					+ session + "'";
			else if(status==0){ sqlUpdate="UPDATE " + database_customer + " SET block0=" + time + ", total_mo=total_mo+1 WHERE user_id='" + user_id + "' AND session='"
			+ session + "'";}
			else {
				sqlUpdate="UPDATE " + database_customer + " SET blockdk=" + time + ", total_mo=total_mo+1 WHERE user_id='" + user_id + "' AND session='"
				+ session + "'";
			}
