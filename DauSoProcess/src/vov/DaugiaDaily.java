...
public class DaugiaDaily extends Thread {
	...
	private int getcurrentPhien() {
		...
		String query0 = "select phien from daugia_phien where current_timestamp > "
	...
	private String[] getWinner() {
		...
		try {
			...
			String query0 = "select * from daugia_phien where adddate(current_timestamp,-7) > "
					+ " begintime and adddate(current_timestamp,-7) < endtime";
			statement = connection.prepareStatement(query0);
			if (statement.execute()) {
				rs = statement.getResultSet();
	...
	private int saveWinner(String gameid, String keyword, String serviceid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winner (game_id,command_code,user_id,service_id,info,sequence) values ('"
					+ gameid
					+ "','"
					+ keyword
					+ "','"
