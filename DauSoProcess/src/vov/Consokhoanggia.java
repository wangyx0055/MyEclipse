...
public class Consokhoanggia extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String info = "Con so dang chiem uu the nam trong khoang $T1 den $T2. De biet con so ban muon da co nguoi dat chua, soan: "
								+ keyword.getKeyword()
								+ " conso gui 8551";
			...
			if (sTokens.length > 1) {
				...
				if (count.equalsIgnoreCase("0")) {
					sMTReturn = "Con so ban dua ra chua co ai dat, hay nhanh tay soan: "
								+ keyword.getKeyword()
								+ " conso gui 8751 de so huu "
							+ sSanpham;

				} else {

					sMTReturn = "Con so ban dua ra da co "
							+ count
							+ "  khach hang dat trung, hay dua ra cac muc gia khac va soan tin "
								+ keyword.getKeyword()
								+ " conso gui 8751 ";
				...
				int randomMin = 10+ random.nextInt(50);

				int randomMax = 10+ random.nextInt(50) ;
	...
	private static String countPrice(String phien, String price) {
		...
		ResultSet rs = null;
		String query = "select count(*) from conso_sieure where phien="
