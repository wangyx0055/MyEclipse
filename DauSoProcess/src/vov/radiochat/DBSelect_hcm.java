...
public boolean checkNick(String nick){
		...
		try {
			...
			if (stmt.execute()) {
				...
				Util.logger
						.error("dbManager - search id  CT HCM : execute Error!!");
			}
		} catch (SQLException ex3) {
			Util.logger
					.error("dbManager - search id  CT HCM. SQLException:"
							+ ex3.toString());
			Util.logger.printStackTrace(ex3);
		} catch (Exception ex2) {
			Util.logger
...
public String getNick(String UserID){
	...
	try {
		...
		if (stmt.execute()) {
			...
			Util.logger
					.error("dbManager - search id of userID  CT HCM : execute Error!!");
		}
	} catch (SQLException ex3) {
		Util.logger
				.error("dbManager - search id of userID CT HCM. SQLException:"
						+ ex3.toString());
		Util.logger.printStackTrace(ex3);
	} catch (Exception ex2) {
		Util.logger
...
public int getID(String UserID){
	...
	String sqlQuery = "SELECT id FROM vovhcm_chat_users where user_id = '" + UserID +"' ";
