				...
				{						
					try
						...
						try
						{
							...
							Util.logger.info ("start add1____" + strQuestion1_start[4]);
							Util.logger.info ("start add2____" + strQuestion1_start[5]);
							Util.logger.info ("start add3____" + strQuestion1_start[6]);
							Util.logger.info ("end____" + strQuestion1_start[7]);
							...
							Timestamp question_add1 = Timestamp.valueOf(strQuestion1_start[4]);
							Timestamp question_add2 = Timestamp.valueOf(strQuestion1_start[5]);
							Timestamp question_add3 = Timestamp.valueOf(strQuestion1_start[6]);
							Timestamp end = Timestamp.valueOf(strQuestion1_start[7]);
							 ...
							 if(commandCode.equalsIgnoreCase("AN"))							
							{
								...
								else if( time.getTime() >= question3_start.getTime() && time.getTime() <=question_add1.getTime())
								{
								...
								else if( time.getTime() >= question_add1.getTime() && time.getTime() <=question_add2.getTime())
								{
									dbInsert.UpdateTovovhcmfunwithmusic_answer(userID, "stage_add1", songname, session);
								...
								else if( time.getTime() >= question_add2.getTime() && time.getTime() <=question_add3.getTime())
								{
									dbInsert.UpdateTovovhcmfunwithmusic_answer(userID, "stage_add2", songname, session);
									msgObject.setUsertext("Dap an cua ban la " + songname +". Hay tiep tuc theo doi de biet ket qua. Du doan cang nhieu co hoi trung thuong cang lon. DTHT 1900571566");
									msgObject.setMsgtype(1);
									msgObject.setContenttype(0);
									DBUtil.sendMT(msgObject);
								}
								else if( time.getTime() >= question_add3.getTime() && time.getTime() <=end.getTime())
								{
									dbInsert.UpdateTovovhcmfunwithmusic_answer(userID, "stage_add3", songname, session);
									msgObject.setUsertext("Dap an cua ban la " + songname +". Hay tiep tuc theo doi de biet ket qua. Du doan cang nhieu co hoi trung thuong cang lon. DTHT 1900571566");
									msgObject.setMsgtype(1);
									msgObject.setContenttype(0);
									DBUtil.sendMT(msgObject);
							...
							{
								if (time.getTime() >= start.getTime() && time.getTime() <=question_add1.getTime())
								{									
