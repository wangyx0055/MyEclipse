...
public class Daugiaovovtv extends ContentAbstract {
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(1) from (select * from daugia_sieurevovtv where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugia_sieurevovtv where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_sieurevovtv (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			if (statement.executeUpdate() != 1) {
				Util.logger
						.info("daugia_sieurevovtv: Insert into daugia_sieurevovtv Failed");
			}
			String query1 = "select price from daugia_sieurevovtv where user_id='"
					+ userid + "'";
			statement = connection.prepareStatement(query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugia_sieurevovtv where phien="
							+ phienid;
					statement = connection.prepareStatement(query2);
					...
					String query3 = "select count(1) from daugia_sieurevovtv where price ="
				...
				String query4 = "select count(1) from daugia_sieurevovtv where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugia_winnervovtv where phien ="
					+ phien;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugia_sieurevovtv where phien ="
	...
	private static String[] getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT phien,begintime,endtime,sanpham FROM daugia_phienvovtv  where isprocess=1 ";
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winnervovtv (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.info("SieuRe: Insert in to daugia_winnervovtv Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winnervovtv  SET command_code ='"
	...
	private static boolean updateDuynhat(int phien, int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_phienvovtv  SET duynhat ='" + price
					+ " ' WHERE phien=" + phien;
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadauvovtv  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winnervovtv  WHERE phien="
					+ phien;
