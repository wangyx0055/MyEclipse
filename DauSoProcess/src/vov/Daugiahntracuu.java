...
public class Daugiahntracuu extends ContentAbstract {
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				if (count.equalsIgnoreCase("0")) {
					sMTReturn = "Muc gia ban dua ra chua co ai dat, hay nhanh tay soan: DG giadau gui 8751 de so huu "
							+ sSanpham;
					...
					sMTReturn = "Muc gia ban dua ra da co "
							+ count
							+ "  khach hang dat trung, hay dua ra cac muc gia khac va soan tin DG giadau gui 8751 ";
	...
	private static String countPrice(String phien, String price) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "select count(*) from daugia_sieure where phien="
