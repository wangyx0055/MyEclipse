...
public class Daugiahcm2_sai extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			Util.logger.sysLog(2, this.getClass().getName(), "options: "
					+ options);
			String stime = "Chuong trinh chua bat dau.Hay quay lai vao chuong trinh lan sau vao 19h 30 hang tuan tren VOV giao thong 91MHz.DTHT:1900571566";
			...
			if (sTokens.length == 1) {
				...
				msgObject.setUsertext("De biet khoang gia dang chiem uu the,soan: DA gui 8551.De biet muc gia ban muon da co ai dat chua,soan:DA gia gui 8551.DTHT:1900571566");
		...
		if (iPrice > 0) {
			...
			if ((iPrice == Integer.parseInt(gw[1]))
				...
				mtReturn = "Ban da dat gia la "
						+ iPrice
						+ "000d.Muc gia ban dat dang la thap nhat va duy nhat.Nhanh tay dat gia de gianh quyen mua "
						+ sSanpham + " voi gia thap nhat";
			...
			if (iNum == 0) {
				if (isEqual == 1) {
					if (gw[1].equalsIgnoreCase("-"))
						gw[1] = "10000";
					num = Countcantren(iPrice + "", gw[1]);
						mtReturn = "Muc ban tra "
							+ iPrice
							+ "000d la thap nhat nhung khong phai duy nhat.Tren muc nay con "
							+ num
							+ " muc gia thap chua ai dat. Nhanh dat gia khac de so huu "
							+ sSanpham;
				...
				if (isEqual == 1) {
					...
					num = Countcanduoi(iPrice + "");
				mtReturn = "Muc ban tra "
							+ iPrice
							+ "000d la duy nhat nhung khong phai thap nhat.Duoi muc gia nay con "
							+ num
							+ " muc gia thap chua ai dat. Nhanh dat gia khac de so huu "
							+ sSanpham;
				}
	...
	private String[] checkSession() {
		...
		try {
			...
			String query2 = "select * from daugia_phienhcm where current_timestamp >"
			...
			if (statement.execute()) {
				...
				if (rs.next()) {
					...
					DBUtil
							.executeSQL(
									connection,
									"insert into daugia_phienhcm(phien,begintime,endtime) select phien + 1,adddate(begintime,7),adddate(endtime,7)  from daugia_phienhcm where phien="
											+ (phien - 1));
				}
			}
			dbpool.cleanup(rs, statement);

			String query5 = "select begintime,endtime,sanpham from daugia_phienhcm where phien="
					+ phien;
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(1) from (select * from daugia_sieurehcm where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugia_sieurehcm where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_sieurehcm (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			if (statement.executeUpdate() != 1) {
				Util.logger
						.info("daugia_sieurehcm: Insert into daugia_sieurehcm Failed");
			}
			String query1 = "select price from daugia_sieurehcm where user_id='"
					+ userid + "'";
			...
			if (statement1.execute()) {
				...
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugia_sieurehcm where phien="
							+ phienid;
					...
					String query3 = "select count(1) from daugia_sieurehcm where price ="
				...
				String query4 = "select count(1) from daugia_sieurehcm where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugia_winnerhcm where phien ="
					+ phien;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugia_sieurehcm where phien ="
	...
	private static String[] getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT phien,begintime,endtime,sanpham FROM daugia_phienhcm  where isprocess=1 ";
	...
	private static String Countcanduoi(String price) {
		String result = "0";
		Connection cnn = null;
		PreparedStatement statement = null;
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadauhcm  where id>=1 and id<= "
				+ price;
	...
	private static String Countcantren(String price, String winprice) {
		String result = "0";
		Connection cnn = null;
		PreparedStatement statement = null;
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT count(1) FROM giadauhcm  where id>=" + price
				+ " and id<= " + winprice;
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winnerhcm (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.info("SieuRe: Insert in to daugia_winnerhcm Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winnerhcm  SET command_code ='"
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadauhcm  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winnerhcm  WHERE phien="
					+ phien;
