...
public class CopyOfSieutoc extends ContentAbstract {
	String GAMEID = "SIEUTOC";
	String INFOCS = "So huu OMNIA trong tam tay: TE gui 8551";
	...
	private String mtReturn(String userid, String serviceid, String keyword,
		...
		if (isNewSession == 1) {
			...
			String sMTInform = "Doi thu " + sHiddenUser + " da doat "
					+ sSanpham + " cua ban, ban da giu "
					+ last_totaltime + " giay. Soan: " + sLastKeyword
					+ " gui " + sServiceid + ". Ket thuc phien 24h "
		...
		try {
			...
			if (result3.size() > 0) {
				for (int i = 0; i < result3.size(); i++) {
					...
					Vector item = (Vector) result3.elementAt(i);
			...
			String query = "select * from icom_stduser where session_id="
					+ phien;
	...
	private int insertData(String keyword, String serviceid, String userid,
		...
		String sqlInsert = "Insert into icom_stduser (command_code,service_id,user_id,lasttime,date,totaltime,session_id,operator) values ('"
				+ keyword
				+ "','"
				+ serviceid
				+ "','"
	...
	private String[] getInfo(String keyword, String serviceid, String userid,
		...
		try {
			...
			String query = "select * from icom_stduser where session_id="
					+ sessionid;
			Vector result = DBUtils.getVectorTable(connection, query);
			if (result.size() > 0) {// La MO dau tien cua chuong trinh
				// String query1 = "select * from vmg_vnnlinks_sdtuser where
				// command_code='"
				// + keyword.toUpperCase()
				// + "' and service_id='"
				// + serviceid + "' and user_id='" + userid + "'";
				// Kiem tra co phai la MO dau tien cua KH khong?
				String query1 = "select * from icom_stduser where user_id='"
				...
				String query2 = "select user_id,operator from icom_stduser where session_id="
						+ sessionid
						+ " and lasttime in (select max(lasttime) from icom_stduser where session_id="
						+ sessionid + ")";
				...
				if (result2.size() > 0) {
					//for (int i = 0; i < result2.size(); i++) {
						...
						// String[] record = (String[]) result2.get(i);
						// lastUser = record[0];
						// operator_lastuser = record[1];
					//}
				}
				// Tim thoi gian update sau cung cua khach hang lien truoc
				// do
				String query3 = "select max(lasttime)from icom_stduser where service_id='"
						+ serviceid
						+ "' and user_id='"
						+ lastUser
				...
				if (result3.size() > 0) {
					for (int i = 0; i < result3.size(); i++) {
						// String[] record = (String[]) result3.get(i);
						// last_lasttime_temp = record[0];

						Vector item = (Vector) result3.elementAt(i);
						last_lasttime_temp = (String) item.elementAt(0);
						// operator_lastuser = (String) item.elementAt(1);
					}
				}
				// Tim thoi gian KH da chiem giu (chua cong voi khoang thoi
				// gian moi)
				String query4 = "select totaltime,id,command_code from icom_stduser where service_id='"
						+ serviceid
						+ "' and user_id='"
						+ lastUser
						+ "'and lasttime ='"
						+ last_lasttime_temp
						+ "' and session_id=" + sessionid;
				Vector result4 = DBUtils.getVectorTable(connection, query4);
				if (result4.size() > 0) {
					for (int i = 0; i < result4.size(); i++) {
						// String[] record = (String[]) result4.get(i);
						// last_totaltime_temp = record[0];

						Vector item = (Vector) result4.elementAt(i);
						last_totaltime_temp = (String) item.elementAt(0);
						last_id_temp = (String) item.elementAt(1);
						lastKeyword = (String) item.elementAt(2);
					}
				}
				String query5 = "select max(lasttime)from icom_stduser where  service_id='"
						+ serviceid
						+ "' and user_id='"
						+ userid
						+ "'and session_id=" + sessionid;
				Vector result5 = DBUtils.getVectorTable(connection, query5);
				if (result5.size() > 0) {
					for (int i = 0; i < result5.size(); i++) {
						// String[] record = (String[]) result5.get(i);
						// current_lasttime_temp = record[0];

						Vector item = (Vector) result5.elementAt(i);
						current_lasttime_temp = (String) item.elementAt(0);
					}
				}
				String query6 = "select totaltime,id from icom_stduser where  service_id='"
						+ serviceid
						+ "' and user_id='"
						+ userid
						+ "'and lasttime ='"
						+ current_lasttime_temp
						+ "' and session_id=" + sessionid;
				Vector result6 = DBUtils.getVectorTable(connection, query6);
				if (result6.size() > 0) {
					for (int i = 0; i < result6.size(); i++) {
						// String[] record = (String[]) result6.get(i);
						// current_totaltime_temp = record[0];
						// id_temp = record[1];

						Vector item = (Vector) result6.elementAt(i);
						current_totaltime_temp = (String) item.elementAt(0);
						current_id_temp = (String) item.elementAt(1);
					}
				}
			...
			Util.logger
					.sysLog(2, this.getClass().getName(),
							"Sieutocdo:Get vmg_vnnlinks_stduser Failed"
									+ ex.toString());
			return null;
		} finally {
			dbpool.cleanup(connection);
	...
	private int updateData(Timestamp current_timestamp, long totaltime, int id) {
		...
		String sqlUpdate = "update icom_stduser set lasttime='"
				+ current_timestamp + "',totaltime=" + totaltime
				+ " where id =" + id;
	...
	public static void main(String[] args) {
		...
		long milliSecond = getCurrentMiliSec();// System.currentTimeMillis();
		String last_lasttime_temp = "2008-12-16 13:00:00.0";
