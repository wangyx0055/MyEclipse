...
public class DangkydanthuMC extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		String dbcontent = "dangky_danthumc";
		String filmname = " thu lam MC";
		String operator = msgObject.getMobileoperator();
		String stime = "2011-05-15 23:59:00";
		...
		try {
			...
			if (time.getTime() > endtime.getTime()) {
				msgObject
						.setUsertext("Xin loi thoi han dang ky tham gia chuong trinh "
								+ filmname
			...
			if (sTokens.length == 1) {
				if (result[1] != 0) {
					// mt1
					msgObject
							.setUsertext("C.on ban da dag ky chuong trinh"
									+ filmname
									+ ".SBD cua ban:"
									+ fomatSBD(result[1])
					...
					// mt2
					msgObject
							.setUsertext("Moi thong tin hop le thi sinh phai dang ky truoc khi dan thu MC qua 2 cach goi 19001515 hoac truy cap web: http://ifim.vn");
						...
						if (giff) {
							...
							if (result[1] != 0) {
								sendGifMsg(
										...
										"Ban vua dc sdt"
												+ userid
												+ "gui tang dang ky tham gia chuong trinh"
									+ filmname
									+ ".SBD cua ban:"
												+ fomatSBD(result[1])
