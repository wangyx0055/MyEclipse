	...
	try {
		...
		if (stmt.execute()) {
			...
			while (rs.next()) {
				...
				result = result + rs.getString("end_timequestion2");
...
public String player_boy(){
	...
	String sqlQuery = "SELECT player_boy FROM vovhcm_chat_answer_couple where session_id in ( select session_id from vovhcm_chat_time where status =1 ) and action = 0";
...
public String player_girl(){
	...
	String sqlQuery = "SELECT player_girl FROM vovhcm_chat_answer_couple where session_id in ( select session_id from vovhcm_chat_time where status =1 ) and action = 0";
