 ...
 * @version 1.0
 */
public class Cau8051 extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length !=2) {
				...
				msgObject
						.setUsertext("Tin nhan sai cu phap. DTHT 1900571566.");
			...
			if (sTokens[1].equalsIgnoreCase("MB")) {
				result = getResultCau(1);
			...
			if (sTokens[1].equalsIgnoreCase("MT")) {
			...
			for (int i = 0; i < 3; i++) {
				company_name = getCompanyName(list_companyid[i]);
					...
					result = getResultCau(list_companyid[i]);
					Util.logger.info("result:" + result[0]);
	...
	public int[] getCompanyIDMT() {
		...
		try {
			...
			String sqlSelect = "select RESULT_COMPANY_ID from LOTTERY_CAU_FULL  where  (to_char(result_date,'dd/mm/yyyy')=to_char(sysdate,'dd/mm/yyyy') or to_char(result_date,'dd/mm/yyyy')=to_char(sysdate-1,'dd/mm/yyyy')) and rownum<=3 and RESULT_COMPANY_ID in (281,271,311,301,331,251,261,341,351,211,321,241,221,291)";
			stmt = conn.prepareStatement(sSql,
					ResultSet.TYPE_SCROLL_INSENSITIVE,
					ResultSet.CONCUR_READ_ONLY);
	...
	public int[] getCompanyIDMN() {
		...
		try {
			...
			sSql = "select RESULT_COMPANY_ID from LOTTERY_CAU_FULL  where  (to_char(result_date,'dd/mm/yyyy')=to_char(sysdate,'dd/mm/yyyy') or to_char(result_date,'dd/mm/yyyy')=to_char(sysdate-1,'dd/mm/yyyy')) and rownum<=3 and RESULT_COMPANY_ID in (21,31,141,151,101,81,91,51,61,71,111,121,131,201,191,161,171,231,181,11,41)";
