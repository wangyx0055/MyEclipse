...
public class TextbaseZF extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				String HOROSCOPE = getStringfromHashMap(_option, "theloai",
						"truyencuoi");

				String mt = getContent(HOROSCOPE);
				...
				for (int j = 0; j < mtsplit.length; j++) {
					if (!"".equalsIgnoreCase(mtsplit[j])) {
						String utfStr = mtsplit[j].replace(" ", "%20");
						 utfStr = utfStr.replace("&", "va");
						...
						if (SoapZonefreeMT.getMessages(msgObject, total, index,
								ismore)) {
							Util.logger.info("OK");
						} else
	...
	private static String getContent(String cung) {
		Connection cnn = null;
		...
		try {
			...
			String query = "SELECT content FROM icom_textbase_data where gameid='"+cung+"' order by rand() limit 1 ;";
