...
public class Huynick extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (nick.equalsIgnoreCase("")) {
				msgObject
						.setUsertext("Ban chua dang ky Chat tren Hop Nhac So VTV6.Soan: IN nickchat gui 8751 de dky nick va tham gia Chat cung ban be. DTHT: 1900571566");
				...
				msgObject
						.setUsertext("Nick "
								+ nick
								+ " da duoc huy. De dang ky nick khac,soan: IN nick gui 8751. Binh chon bai hat soan: VO tenbaihat gui 8751");
				msgObject.setMsgtype(1);
				msgObject.setContenttype(0);
				DBUtil.sendMT(msgObject);
				Thread.sleep(1000);
	...
	private static boolean deleteUser(String user) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM NickName  WHERE [MSISDN]='" + user +"'";
	...
	private static String Getnickbysdt(String sdt) {
		...
		try {
			...
			String sqlSelect = "SELECT [NickName] FROM [NickName] where [MSISDN]='"
					+ sdt + "' ";
