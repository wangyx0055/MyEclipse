...
public class votebaihat extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length == 1) {
				...
				if (isforwardMO.equalsIgnoreCase("1")) {
					try {
						String result = MoSenderMediatech.SendXML(msgObject
								...
								susertext, msgObject.getMobileoperator());
				...
				code = findCodebyName(tenbaihat,date);
				if (code[0].equalsIgnoreCase("")) {

					code = findCode(tenbaihat,date);
				}
				if (code[0].equalsIgnoreCase("")) {

					code = findCodebyCode(tenbaihat,date);
				}
				if (!code[0].equalsIgnoreCase("")) {
					...
					msgObject.setUsertext("Cai dat bai hat " + code[0]
							+ "  lam nhac cho soan NC " + code[0]
							+ "  Gui 8751. Lien he 1900571566 de duoc ho tro");
					msgObject.setMsgtype(0);
					msgObject.setContenttype(0);
					DBUtil.sendMT(msgObject);
					Thread.sleep(1000);
					...
					msgObject.setUsertext("Cac bai hat Hot tren iMbox:"
							+ findListofMusic(date).substring(0,
									findListofMusic(date).length() - 1)
	...
	private static String[] findCode(String musicname,String date) {
		String[] result = new String[2];
		String result1 = "";
		String result2 = "";
		...
		try {
			...
			String sqlSelect = "select top 1 replace(NameVote,' ','') NameVote,Code from PlaysList where replace(NameVote,' ','') like '"
					+ musicname + "%' and DatePlay='"
					+ date + "'   order by newid() ; ";
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					result1 = rs.getString(1).substring(0,
							rs.getString(1).length() - 4);
	...
	private static String[] findCodebyCode(String code,String date) {
		String[] result = new String[2];
		String result1 = "";
		String result2 = "";
		...
		try {
			...
			String sqlSelect = "select top 1 replace(NameVote,' ','') NameVote,Code from PlaysList where Code ='"
					+ code + "' and DatePlay='"
					+ date + "'  order by newid() ; ";
	...
	private static String[] findCodebyName(String musicname,String date) {
		String[] result = new String[2];
		String result1 = "";
		String result2 = "";
		// String musicid = "";
		...
		try {
			...
			String sqlSelect = "select top 1 replace(NameVote,' ','') NameVote,Code from PlaysList where SUBSTRING(replace(NameVote,' ',''),0,len(replace(NameVote,' ',''))-3) ='"
					+ musicname + "' and DatePlay='"
					+ date + "'  order by newid() ; ";
