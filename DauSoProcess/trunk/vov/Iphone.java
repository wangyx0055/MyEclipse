...
public class Iphone extends ContentAbstract {
	...
	private static String getResult(String userid, int session) {
		...
		ResultSet rs = null;
		String query = "SELECT result FROM icom_iphone WHERE user_id= '"
	...
	private static int getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT session FROM iphone_session  where isprocess=1 ";
	...
	private String getResult(int session) {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "select result from iphone_session where session="
				+ session;
