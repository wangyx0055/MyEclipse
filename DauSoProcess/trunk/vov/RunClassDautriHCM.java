...
public class RunClassDautriHCM extends Thread {
	...
	private Hashtable getTime() {
		...
		try {
			...
			String sqlSelect = "SELECT id,current,start_dk ,finish_dk ,start_time, block1, block2, block3, block4, block5, block6,block7,block8,block9,block10, end_time, isprocess FROM icom_dautrihcm_config WHERE active = 1";
			...
			statement = connection.prepareStatement(sqlSelect);
	 	     if (statement.execute()) {
				...
				while (rs.next()) {
					...
					secretNumber.put("end", result[15]);
					result[16] = rs.getString(17);
					secretNumber.put("isprocess", result[16]);
				}
