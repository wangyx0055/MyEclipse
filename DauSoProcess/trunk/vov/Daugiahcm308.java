...
//HAPTT
public class Daugiahcm308 extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		if (check[0].equalsIgnoreCase("1")) {
			...
			if (msgObject.getMobileoperator().equalsIgnoreCase("GPC")||msgObject.getMobileoperator().equalsIgnoreCase("BEELINE")||msgObject.getMobileoperator().equalsIgnoreCase("EVN"))
			{
				msgObject.setUsertext("So luong tin nhan cua ban da vuot qua muc gioi han trong thoi gian qui dinh theo qui dinh cua nha mang.Nen tin nhan cua ban khong co hieu luc.");
					
			}
			else
			{	
			msgObject.setUsertext("Theo qui dinh cua nha mang,so luong tin nhan cua ban da vuot qua so luong tin nhan cho phep trong mot ngay.Nen tin nhan cua ban khong con hieu luc.");
			}
		...
		// mt2
		msgObject
				.setUsertext("Tu 27/07 den 19/9 Dky Mua Laptop tai TragopLaptop.com, Khuyen mai Lai suat =0% + The Bao tri 3 nam mien phi");
		...
		{
		bgia=Banggia(iPhien);
		 Set set = bgia.entrySet();
		  Iterator i = set.iterator();
		    while(i.hasNext()){
		      Map.Entry me = (Map.Entry)i.next();
		     int v=(Integer) me.getValue();
		     int k=(Integer) me.getKey();
		     Util.logger.info("tim gia"+k +"va "+v);
		      if (k!= v )
		      { updateDuynhat(iPhien,k);
		      return messages;
		      }
		    }
			return messages;
			...
			if (statement.execute()) {
				...
				if (rs.next()) {
					...
					DBUtil
							.executeSQL(
									connection,
									"insert into daugia_phienhcm(phien,begintime,endtime) select phien + 1,adddate(begintime,7),adddate(endtime,7)  from daugia_phien where phien="
											+ (phien - 1));
				}
			}
			dbpool.cleanup(rs, statement);
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(*) from (select * from daugia_sieurehcm where phien="
					+ phien
					+ ") as T1 group by price having count(*) = 1) as T2";
			statement = connection.prepareStatement(query1);
			Util.logger.error("select price:" + query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
			...
			if (!"".equals(WinnerPrice)) {
				...
				statement = connection.prepareStatement(query2);
				Util.logger.error("select user win:" + query2);
				if (statement.execute()) {
					rs = statement.getResultSet();
					while (rs.next()) {
		...
		try {
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					String query3 = "select count(*) from daugia_sieurehcm where price ="
				...
				String query4 = "select count(*) from daugia_sieurehcm where price <"
						+ price + " and phien=" + phienid;
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.error("SieuRe: Insert in to daugia_winner Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateDuynhat(int phien,int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_phienhcm  SET duynhat ='"
