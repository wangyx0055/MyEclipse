...
public class Daugiatuan extends ContentAbstract {
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			Util.logger.sysLog(2, this.getClass().getName(), "options: "
					+ options);
			String stime = " 6h thu2 den 18h chu nhat ";
			...
			int iPhien = 0;
			String[] check = spam.DBUtils2.checkspam(msgObject);
			if (check[0].equalsIgnoreCase("1")) {
				if (spam.DBUtils2.checklistspam(msgObject)) {
					return null;
				} else
					spam.DBUtils2.saverequest(msgObject);
				if (msgObject.getMobileoperator().equalsIgnoreCase("GPC")
					...
					msgObject
							.setUsertext("So luong tin nhan cua ban da vuot qua muc gioi han trong thoi gian qui dinh theo qui dinh cua nha mang.Nen tin nhan cua ban khong co hieu luc.");

				} else {
					msgObject
							.setUsertext("Theo qui dinh cua nha mang,so luong tin nhan cua ban da vuot qua so luong tin nhan cho phep trong mot ngay.Nen tin nhan cua ban khong con hieu luc.");
				}
			...
			if (spam.DBUtils2.checklistspam(msgObject)) {
				spam.DBUtils2.deletelistspam(msgObject);
			}
			...
			if (kq1[0].equalsIgnoreCase("")) {
				msgObject
						.setUsertext("Chuong trinh chua bat dau.Hay quay lai vao chuong trinh lan sau vao "
								+ stime
								+ " tren bao The thao hang ngay.DTHT:1900571566.");
				msgObject.setMsgtype(1);
				messages.add(new MsgObject(msgObject));
				return messages;
			}
			...
			if (sTokens.length == 1) {
				...
				//msgObject
				//		.setUsertext("Mua Tra gop Laptop Lai suat 0%, Tang tien mat len den 1.500.000d cho moi doi tuong. Tai Tragoplaptop.com tu 25/9 den 15/10 HL: 01233.777.111; 01234.777.111");
	...
	private int getPrice(String sTokens[]) {
		...
		Util.logger.info("getPrice" + flag);
		return flag;
	}
	...
	private String[] checkSession() {
		...
		try {
			...
			String query2 = "select * from daugiatuan_phien where current_timestamp >"
			...
			if (statement.execute()) {
				...
				if (rs.next()) {
					...
					DBUtil
							.executeSQL(
									connection,
									"insert into daugiatuan_phien(phien,begintime,endtime) select phien + 1,adddate(begintime,7),adddate(endtime,7)  from daugiatuan_phien where phien="
											+ (phien - 1));
				}
			}
			dbpool.cleanup(rs, statement);

			String query5 = "select begintime,endtime,sanpham from daugiatuan_phien where phien="
					+ phien;
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(*) from (select * from daugiatuan_sieure where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugiatuan_sieure where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugiatuan_sieure (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			if (statement.executeUpdate() != 1) {
				Util.logger
						.info("daugiatuan_sieure: Insert into daugiatuan_sieure Failed");
			}
			String query1 = "select price from daugiatuan_sieure where user_id='"
					+ userid + "'";
			statement = connection.prepareStatement(query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugiatuan_sieure where phien="
							+ phienid;
					statement = connection.prepareStatement(query2);
					...
					String query3 = "select count(*) from daugiatuan_sieure where price ="
				...
				String query4 = "select count(*) from daugiatuan_sieure where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugiatuan_winner where phien ="
					+ phien;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugiatuan_sieure where phien ="
	...
	private static String[] getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT phien,begintime,endtime,sanpham FROM daugiatuan_phien  where isprocess=1 ";
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugiatuan_winner (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
			...
			statement = connection.prepareStatement(sqlInsert);
			if (statement.executeUpdate() != 1) {
				Util.logger.info("SieuRe: Insert in to daugiatuan_winner Failed");
				return -1;
			}
			return 1;
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugiatuan_winner  SET command_code ='"
	...
	private static boolean updateDuynhat(int phien, int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugiatuan_phien  SET duynhat ='" + price
					+ " ' WHERE phien=" + phien;
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
				Util.logger.info("Update gia se la nho nhat va duy nhat:"
						+ price);
				return false;
			}
			return true;
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadautuan  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugiatuan_winner  WHERE phien="
					+ phien;
