 ...
 * @version 1.0
 */
public class Nhacchotructiep extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String inv_telco = "Hien tai dich vu chua ho tro mang cua ban, moi ban quay lai sau.DTHT 1900585836";

			HashMap _option = new HashMap();
			String options = keyword.getOptions();
			String dtbase = "nhaccho";
			String last_msg = "soan tin XNNC makiemtra gui 8042";
			try {
			...
			if (sTokens.length > 1) {
							...
							if (giff_telco.toUpperCase()
								...
								try {
									String result2 = sendXMLRING.SendXML(
											...
											resultMusicID[1], now());
									mt1 = "Ban gui tang chua thanh cong co the do thue bao duoc tang chua cai nhac cho.Xin vui long thu lai. Tang ban top bai hat HOT:";
									if (result2.equalsIgnoreCase("0")) {
										...
										mt1 = "Ban gui tang chua thanh cong do thue bao duoc nhan khong co that hoac chua dang ky nhac cho.Tang ban top bai hat HOT:";
									} else if (result2.equalsIgnoreCase("600")) {
										mt1 = "Ban gui tang chua thanh cong,so dien thoai duoc tang chua dang ky su dung nhac cho. Tang ban top bai hat HOT:";
									} else if (result2.equalsIgnoreCase("604")) {
										mt1 = "Ban gui tang chua thanh cong, thu vien nhac cho cua thue bao gui tang da day. Tang ban top bai hat HOT:";
									} else if (result2.equalsIgnoreCase("200")) {
										mt1 = "Ban gui tang chua thanh cong do chua dang ky nhac cho.Tang ban TOP bai hat HOT";
									}
								...
								try {
									...
									if (result.equalsIgnoreCase("0")) {
										...
										mt1 = "Ban vui long dang ky nhac cho truoc.Soan Dk gui 1221.Tang ban TOP bai hat HOT";
										// try {
										...
										// mt1 =
										// "De gui tang thanh cong,ban phai cai dat nhac cho truoc, vui long "
										// + last_msg
										// + " de xac nhan cai dat.";
				...
				String info2Client = "Tin nhan sai cu phap. DTHT:1900585836";
				msgObject.setUsertext(info2Client);
				msgObject.setMsgtype(1);
				msgObject.setContenttype(0);
				DBUtil.sendMT(msgObject);
	...
	private static String[] findMusicID(String find, String infoid) {
		...
		try {
			...
			String sqlSelect = "SELECT top 1 Name,"
					+ ope
					+ " FROM WAPICOM.DBO.RingbackSearch Inner join WAPICOM.DBO.Ringback ON WAPICOM.DBO.RingbackSearch.RingbackID = WAPICOM.DBO.Ringback.RingbackID WHERE WAPICOM.DBO.RingbackSearch.RingbackName LIKE N'%"
