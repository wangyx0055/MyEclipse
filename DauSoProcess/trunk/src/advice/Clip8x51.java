 ...
 * @version 1.0
 */
public class Clip8x51 extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String mt1= "De tai CLIP theo ma so, soan: CLA maso gui 8751.DTHT 1900571566";
			...
			if (sTokens.length != 1) {
				msgObject.setUsertext("Tin nhan sai cu phap. DTHT:1900571566");
	...
	private static String findListofGame(String dtbase, String userid,
		...
		try {
			...
			String sqlSelect = "SELECT tengame,magame FROM icom_clip WHERE 1=1 ";
			Util.logger.info("last id : " + lastid);
			
			if (!"".equalsIgnoreCase(lastid)) {
				Util.logger.info("nulllllllllllll ma");
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					if (result.length() > 160) {
						...
						updateListGame(userid, lastid, dbcontent, subcode);
						return lresult;
					} else {
						if ("".equalsIgnoreCase(lastid)) {
