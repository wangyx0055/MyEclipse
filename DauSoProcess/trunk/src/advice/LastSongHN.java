...
public class LastSongHN extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			Hashtable lastSong = Threadlastsong.lastestsong.LAST_SONG;
			...
			if (Threadlastsong.lastestsong.isProcess == 0) {
				msgObject
				...
				return messages;
			} else if (Threadlastsong.lastestsong.isProcess == 1) {
			...
			} else if (Threadlastsong.lastestsong.isProcess == 2) {
				msgObject
				...
				messages.add(new MsgObject(msgObject));
				return messages;
			} else if (Threadlastsong.lastestsong.isProcess == 3) {
	...
	private static boolean saveClient(String session, String user_id,
		...
		try {
			...
			String sqlInsert = "INSERT INTO icom_lastsong_customer (session, user_id, service_id, mobile_operator, info, request_id, exactly, block, receive_time, cpid, keyword ) VALUES ('"
					+ session
					+ "','"
					+ user_id
					+ "','"
