...
public class Sodienthoaitaxi extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length > 1) {
				...
				String noidung = getContent(tinh);
	...
	private static String getContent(String tinh) {
		Connection cnn = null;
		PreparedStatement statement = null;
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		...
		try {
			...
			String query = "SELECT noidung FROM sodttaxi WHERE upper(tinh) = '"
					+ tinh.toUpperCase() + "'";
