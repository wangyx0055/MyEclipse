...
public class DaugiaCongDong_Plus extends ContentAbstract{
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String checkLeng = "";
			if (sTokens.length == 1 || sTokens.length <3 || sTokens.length > 3) {
				msgObject
				...
				try {
					...
					if(requestId!= null)
					sqlInsert = "Insert into daugiaplus_queue (USER_ID,SERVICE_ID, MOBILE_OPERATOR, COMMAND_CODE, REQUEST_ID, INFO, receive_date) values ('"
							+ sUserid
							+ "','"
							+ sServiceid
							+ "','"
