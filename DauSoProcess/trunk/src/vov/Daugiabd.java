...
public class Daugiabd extends ContentAbstract {
			...
			if (sTokens.length == 1) {
				msgObject
						.setUsertext("Tin nhan sai cu phap.Soan tin "
								...
								+ " de tham gia chuong trinh Dau gia nguoc.DTHT:19001745");
				msgObject.setMsgtype(1);
				messages.add(new MsgObject(msgObject));
...
// Tim nguoi tra gia thap nhat va duy nhat
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(1) from (select * from daugia_sieurebd where phien="
					+ phien
			...
			if (!"".equals(WinnerPrice)) {
				...
				String query2 = "select user_id from daugia_sieurebd where price = "
	...
	private String[] checkPrice(String userid, String keyword,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_sieurebd (user_id,command_code,service_id,price,phien) values ('"
					+ userid
					+ "','"
					+ keyword
					+ "','"
			...
			String query1 = "select price from daugia_sieurebd where user_id='"
					+ userid + "'";
			statement = connection.prepareStatement(query1);
			if (statement.execute()) {
				rs = statement.getResultSet();
				while (rs.next()) {
					MO1 = "-1";
					// rs = statement.getResultSet();
					String query2 = "select max(price),min(price) from daugia_sieurebd where phien="
							+ phienid;
					statement = connection.prepareStatement(query2);
					...
					String query3 = "select count(1) from daugia_sieurebd where price ="
				...
				String query4 = "select count(1) from daugia_sieurebd where price <"
						+ price + " and phien=" + phienid;
	...
	private String[] getWinnerKQ(int phien) {
		...
		try {
			...
			String query = "select user_id,info from daugia_winnerbd where phien ="
					+ phien;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugia_sieurebd where phien ="
	...
	private static String[] getPhien() {
		...
		DBPool dbpool = new DBPool();
		ResultSet rs = null;
		String query = "SELECT phien,begintime,endtime,sanpham FROM daugia_phienbd  where isprocess=1 ";
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winnerbd (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winnerbd  SET command_code ='"
	...
	private static boolean updateDuynhat(int phien, int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_phienbd  SET duynhat ='" + price
					+ " ' WHERE phien=" + phien;
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
	...
	private static boolean deletePrice(int price) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM giadaubd  WHERE id=" + price;
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winnerbd  WHERE phien="
					+ phien;
