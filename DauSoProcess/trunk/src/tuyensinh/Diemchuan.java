...
public class Diemchuan extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		if (sTokens.length == 1) {
			msgObject
					.setUsertext("Ma Truong khong hop le. Soan: "
							+ keyword.getKeyword()
							+ " <MaTruong> <nam> gui "
							+ keyword.getServiceid()
							+ " de xem Diem Chuan vao truong DH,CD nam 2010. Ho tro: 1900571566");
		...
		String matruong = sTokens[1].trim().toUpperCase();

		String validtentruong = validMaTruong(matruong);
		...
		if ("x".equalsIgnoreCase(validtentruong)) {
			...
			if (giff == true) {
				saverequest(giff_userid, matruong);
				sendGifMsg(msgObject.getServiceid(), giff_userid, giff_telco,
						msgObject.getKeyword(), "Sodt " + msgObject.getUserid()
								+ " da dang ky xem Diem Chuan truong:"
								+ matruong
								+ ". He thong se gui den ban ngay khi co.",
						msgObject.getRequestid(), 0);

				msgObject
						.setUsertext("Ban da dang ky xem Diem Chuan cua truong: "
								+ matruong
								+ " thanh cong cho thue bao "
								+ giff_userid + ".");
				msgObject.setContenttype(0);
				msgObject.setMsgtype(1);
				messages.add(new MsgObject(msgObject));
	...
	private static boolean saverequest(String userid, String code) {
		...
		try {
			...
			String sqlInsert = "Insert into tuyensinh_diemchuan_dk( userid,matruong) values ('"
					+ userid + "','" + code + "')";
