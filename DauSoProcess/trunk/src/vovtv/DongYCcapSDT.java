...
public class DongYCcapSDT extends ContentAbstract{
			...
			if (sTokens.length > 1) {
				...
				if (nick.equalsIgnoreCase("")) {
					...
					Thread.sleep(1000);
						...
						msgObject
								.setUsertext("Ban da dong y cung cap SDT. De biet SDT cua thanh vien khac, soan: SDT <ma so cua thanh vien do> gui 8551. DTHT: 1900571566.");
						msgObject.setMsgtype(1);
						msgObject.setContenttype(0);
						DBUtil.sendMT(msgObject);
	...
	private int UpdateNick(String sdt) throws Exception {
		...
		String proceExe = "UPDATE NickName SET IsViewMSISDN = 1 WHERE MSISDN = '"+sdt+"'";
