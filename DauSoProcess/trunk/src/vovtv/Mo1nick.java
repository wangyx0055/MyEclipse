	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length < 2) {
				...
				if(mo.startsWith("MK")) {
					nickId = sTokens[0].substring(2, sTokens[0].length());
				}
					...
					if (chatuser.equalsIgnoreCase("")) {
						msgObject
								.setUsertext("Ma so thanh vien ban vua chon khong ton tai. Vui long kiem tra lai. DTHT: 1900571566.");
						msgObject.setMsgtype(1);
						DBUtil.sendMT(msgObject);
						Thread.sleep(1000);
						return null;
					}
					String blacklist = Getblacklist(nickId);

					blacklist = blacklist.replaceAll(nickId, "x");
					updateBlacklist(msgObject.getUserid(), blacklist);
					msgObject
							.setUsertext("Ban da mo khoa chat rieng cho "+nickId+". De gui noi dung chat: CR masonguoinhan noidungchat gui 8551. DTHT: 1900571566.");
					msgObject.setMsgtype(1);
					DBUtil.sendMT(msgObject);
					Thread.sleep(1000);
				...
				String blacklist = Getblacklist(sttuser);

				blacklist = blacklist.replaceAll(sTokens[1], "x");
				updateBlacklist(msgObject.getUserid(), blacklist);
				msgObject
						.setUsertext("Ban da mo khoa chat rieng cho "+sTokens[1]+". De gui noi dung chat: CR masonguoinhan noidungchat gui 8551. DTHT: 1900571566.");
				msgObject.setMsgtype(1);
				DBUtil.sendMT(msgObject);
				Thread.sleep(1000);
			}
