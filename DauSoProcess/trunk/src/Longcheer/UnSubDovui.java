...
public class UnSubDovui extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		if (isexist(msgObject.getUserid(), MLIST, sservice, bongda)) {
			...
			insertData2cancel(msgObject.getUserid(), msgObject.getServiceid(),
					...
					sservice);
			String strdel = "delete from mlist_dovui where user_id='"
					+ msgObject.getUserid() + "'";
			Subutil.executeSQL("gateway", strdel);
			Util.logger.info(strdel);

			msgObject
					.setUsertext("Ban huy thanh cong dich vu Dovui. Cam on ban da su dung dich vu. DTHT 1900571566");
