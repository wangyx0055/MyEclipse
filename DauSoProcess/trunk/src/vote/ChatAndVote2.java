...
public class ChatAndVote2 extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String vote_x = "x";
			String vote_x_desc = "x";
			String vote_y = "x";
			String vote_y_desc = "x";
			int vote_z = 1;
			...
			if (_type == 0) {
				...
				if (saverequest(_timeactive, keyword.getKeyword(), keyword
						.getServiceid(), msgObject.getUserid(), vote_x, vote_y,
						vote_z)) {
				...
				if (sTokens.length >= 2) {
					...
					if ("x".equalsIgnoreCase(vote_x_desc)) {
						...
						if (saverequest(_timeactive, keyword.getKeyword(),
								keyword.getServiceid(), msgObject.getUserid(),
								vote_x, vote_y, vote_z)) {
							msgObject.setUsertext(_mtvalid);
							msgObject.setMsgtype(1);
							msgObject.setContenttype(0);
							messages.add(new MsgObject(msgObject));
	...
	private String[] validcode(String code, String votingid) {
