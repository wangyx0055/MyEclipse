 ...
 * @version 1.0
 */
public class Nhacchotructiepcasy extends ContentAbstract {
			...
			if (sTokens.length > 1) {
				...
				if (infoid.equalsIgnoreCase("mobifone")) {
					...
					if (sTokens.length == 2) {
						...
						resultMusicID = findMusicIDbysinger(subTokens2, infoid,sTokens[sTokens.length-1]);
						if ("".equalsIgnoreCase(resultMusicID[1])) {
							resultMusicID = findMusicIDlikebysinger(subTokens2, infoid,sTokens[sTokens.length-1]);
						}
	...
	private static String[] findMusicIDbysinger(String find, String infoid,String singname) {
		...
		try {
			...
			String sqlSelect = "SELECT top 1 RingbackName_E,"
					+ ope
					+ " FROM [IComStore].[dbo].Ringback WHERE SearchRingbackName LIKE '%, "
					+ find + ",%'  and SearchSingerName LIKE '%"+singname+"%' and " + ope + " is not null and " + ope
					+ " <>''";
	...
	private static String[] findMusicIDlikebysinger(String find, String infoid,String singname) {
