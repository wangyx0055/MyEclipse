...
public class Chatketnoihcm extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if (sTokens.length < 3) {
				msgObject
						.setUsertext("Noi dung chatting cua ban chua duoc gui di vui long soan lai tin nhan: HA maso noidungchat gui 8351.DTHT: 1900571566");
				msgObject.setMsgtype(2);
				DBUtil.sendMT(msgObject);
				Thread.sleep(1000);
				return null;
				...
				if (sttuser.equalsIgnoreCase("")) {
					msgObject
							.setUsertext("De ket ban va chatting trong chuong trinh KNYT,ban hay dky de tro thanh thanh vien cua KNYT,bang cach soan tin: HA gioitinh sothich gui 8751");
				...
				if (check.equalsIgnoreCase("")) {
					...
					sendGifMsg(
							...
							"Chuc mung ban da nhan duoc thong diep ket ban tu ma so "
									+ sttuser
									+ " trong ctrinh KNYT.DTHT 1900571566.De tra loi,soan:HA maso noidungchat gui 8351.",
	...
	private static String Getuser(String stt) {
		String result = "";
		// String musicid = "";
		Connection connection = null;
		PreparedStatement statement = null;
		...
		try {
			...
			String sqlSelect = "SELECT userid FROM ketnoihcm where stt=" + stt;
	...
	private static String Getstt(String userid) {
		...
		try {
			...
			String sqlSelect = "SELECT stt FROM ketnoihcm where userid='"
					+ userid + "'";
	...
	private static String checkexist(String maso1, String maso2) {
		...
		try {
			...
			String sqlSelect = "SELECT maso1 FROM ketnoichathcm where maso1='"
	...
	private static boolean saveChat(String maso1, String maso2,String noidung) {
		...
		try {
			...
			String sqlInsert = "INSERT INTO ketnoichathcm (maso1,maso2,noidung)"
					+ "VALUES(?,?,?)";
			statement = connection.prepareStatement(sqlInsert);
