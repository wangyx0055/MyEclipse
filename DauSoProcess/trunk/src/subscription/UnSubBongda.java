...
public class UnSubBongda extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		if (arrInfo.length != 2) {
			...
			msgObject.setUsertext("Tin nhan sai cu phap. Soan tin "
					+ msgObject.getKeyword() + " gui "
					+ msgObject.getServiceid()
					+ "  de dang ky dich vu.De huy dich vu soan HUY "
					+ msgObject.getKeyword() + " gui "
					+ msgObject.getServiceid() + ".DTHT 1900571566.");
		...
		if (isexist(msgObject.getUserid(), MLIST, sservice, bongda)) {
			...
			insertData2cancel(msgObject.getUserid(), msgObject.getServiceid(),
					...
					sservice);
			String strdel = "delete from mlist_ttwc where user_id='"
					+ msgObject.getUserid() + "' and upper(service)='BONGDA' AND upper(options)='BONGDA'";
			Subutil.executeSQL("gateway", strdel);
			Util.logger.info(strdel);

			msgObject
					.setUsertext("Ban huy thanh cong dich vu Ban Tin WorldCup. Cam on ban da su dung dich vu. DTHT 19");
			msgObject.setContenttype(0);
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
			return messages;
			...
			msgObject.setUsertext("Ban chua dang ky dich vu. DTHT:1900571566");
			msgObject.setContenttype(0);
			msgObject.setMsgtype(1);
			messages.add(new MsgObject(msgObject));
