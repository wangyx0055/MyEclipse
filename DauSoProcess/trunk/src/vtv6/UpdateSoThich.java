...
public class UpdateSoThich extends ContentAbstract {
			...
			if (sTokens.length == 1) {
				...
				if (content.length() > 160) {
					...
					if (infor.size() == 0) {
						...
						if(checkUser(userid)) {
							int update = UpdateNick(sothich, userid);
							if(update==1) {
								msgObject
								.setUsertext("So thich cua "+ infor.get(1)+" da duoc thay doi. De thong tin duoc hien thi tren Hop Nhac So mot lan nua, soan IC gui 8551. DTHT: 1900571566");
								msgObject.setMsgtype(1);
								msgObject.setContenttype(0);
								DBUtil.sendMT(msgObject);
							}
	...
	private int UpdateNick(String sothich, String sdt) throws Exception {
		int result = 0;
		Connection connection = null;
		CallableStatement callStmt = null;
		ResultSet rs = null;
		...
		String proceExe = "UPDATE NickName SET Favorites= '" +sothich + "' WHERE MSISDN = '"+sdt+"'";
