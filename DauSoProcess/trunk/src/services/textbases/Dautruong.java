...
public class Dautruong extends ContentAbstract {
	String GAMEID = "DAUTRUONG";
	String INFOCS = "\n*HT:0435561862\n*www.bkc.vn han hanh tai tro";
	String INFOCS_FULL = "\n*HT:0435561862\n*www.bkc.vn han hanh tai tro chuong trinh";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			if ( sUserid.equalsIgnoreCase("84972210607") || sUserid.equalsIgnoreCase("841228200028")) {
				return null;
			}
	...
	private String mtReturn(String userid, String serviceid, String keyword,
		...
		if ("xxx".equalsIgnoreCase(sSanpham)) {
			sMTReturn = "Phien choi DAU TRUONG MOBILE ki nay chua bat dau, moi ban quay tro lai sau."
					+ INFOCS;
			return sMTReturn;
		}
		...
		if (isNewSession == 1) {
			sMTReturn = "Ban la nguoi dau tien tham gia DAU TRUONG.Soan: "
					+ sKeyword
					+ " gui "
					+ sServiceid
					+ " de tiep tuc chiem giu. Ket thuc phien 24h "
					+ sEndDate + "" + INFOCS;
			...
			String sMTInform = "Doi thu " + sHiddenUser + " da doat "
					...
					+ " giay, hay soan: " + sLastKeyword + " gui " + sServiceid
					+ ". Ket thuc: 24h " + sEndDate.substring(0, 5)
					+ INFOCS;
			...
			// sMTInform);
			if (isTheFirstMO == 1) {
				sMTReturn = "Chao mung ban tham gia DAU TRUONG.Ban la nguoi dau tien chiem giu san pham. Soan:"
						+ sKeyword
						+ " gui "
						+ sServiceid
						+ " de giu san pham. Het phien: 24h "
						+ sEndDate.substring(0, 5);
				insertData(sKeyword, sServiceid, sUserid, current_timestamp,
						current_timestamp, 0, iPhien, operator);
			} else {
				if (isMO1 == 1) {
					sMTReturn = "Chao mung ban tham gia DAU TRUONG. Ban doat "
							+ sSanpham + " tu doi thu " + sHiddenLastUser
							+ ". Ket thuc phien 24h "
							+ sEndDate.substring(0, 5)  + INFOCS_FULL;
	...
	public String[] checkSession(String gameid) {
		...
		try {
			...
			Util.logger.sysLog(2, this.getClass().getName(), "Dautruong:"
					+ phien);
			String query2 = "select * from icom_dmphien_dautruong where current_timestamp >"
			...
			String query3 = "select sanpham,endtime from icom_dmphien_dautruong where current_timestamp >= begintime and phien="
					+ phien;
			...
			String query = "select count(*) from icom_dautruong_user where session_id="
					+ phien;
			...
			Util.logger.sysLog(2, this.getClass().getName(),
					"Dau truong: Get icom_phien_dautruong Failed" + ex.toString());
	...
	private int insertData(String keyword, String serviceid, String userid,
		...
		String sqlInsert = "Insert into icom_dautruong_user (command_code,service_id,user_id,lasttime,date,totaltime,session_id,operator,milisec) values ('"
				+ keyword
				+ "','"
				+ serviceid
				+ "','"
		...
		try {
			...
			if (DBUtils.executeSQL(connection, sqlInsert) < 0) {
				Util.logger.sysLog(2, this.getClass().getName(),
						": uppdate Statement: Insert  icom_dautruong_user Failed");
				ireturn = -1;
			}
		} catch (Exception ex) {
			Util.logger.sysLog(2, this.getClass().getName(),
					":Insert  icom_dautruong_user Failed");
			Util.logger.printStackTrace(ex);
			ireturn = -1;
		} finally {
			dbpool.cleanup(connection);
	...
	private String[] getInfo(String keyword, String serviceid, String userid,
		...
		try {
			...
			if (isFirstMO != 1) {// La MO dau tien cua chuong trinh
				...
				String query1 = "select id,totaltime,lasttime from icom_dautruong_user where user_id='"
				...
				String query3 = "select max(milisec) from icom_dautruong_user where session_id="
						+ sessionid;
				...
				String query2 = "select user_id,operator,lasttime,totaltime,id,command_code from icom_dautruong_user where session_id="
						+ sessionid
						+ " and milisec = "
						+ tempMilisec
						+ " order by id desc";
			...
			Util.logger.sysLog(2, this.getClass().getName(),
					"Dautruong:Get icom_dautruong_user Failed" + ex.toString());
			Util.logger.printStackTrace(ex);
			return null;
		} finally {
			dbpool.cleanup(connection);
	...
	private int updateData(Timestamp current_timestamp, long totaltime, int id) {
		...
		String sqlUpdate = "update icom_dautruong_user set lasttime='"
		...
		try {
			...
			if (DBUtils.executeSQL(connection, sqlUpdate) < 0) {
				Util.logger.sysLog(2, this.getClass().getName(),
						": uppdate Statement: Update icom_dautruong_user Failed");
				ireturn = -1;
			}
		} catch (Exception ex) {
			Util.logger.sysLog(2, this.getClass().getName(),
	...
	private int updateData(long totaltime, int id) {
		int ireturn = 1;
		String sqlUpdate = "update icom_dautruong_user set totaltime=" + totaltime
				+ " where id =" + id;
