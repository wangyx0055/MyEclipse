...
public class SoapEBC  {
	...
	throws InterruptedException
		...
		while (iRetries > 0) {
			try {
				String url = "http://123.30.188.223:10086/IComMOReceiver.aspx?";
				
				url = Constants._prop.getProperty("url_ebc", url);
				
				String msisdn=msgObject.getUserid();
				String sms=msgObject.getUsertext().replace(" ", "+");
				
			    url=url+"moId="+msgObject.getRequestid()+"&operatorName="+msgObject.getMobileoperator()+"&userId="+msisdn+"&serviceId="+msgObject.getServiceid()+"&commandCode="+msgObject.getKeyword()+"&message="+sms+"&username=iComSMS&password=iComHanoi123%^";
