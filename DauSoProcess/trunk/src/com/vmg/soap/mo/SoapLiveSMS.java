...
public class SoapLiveSMS  {
	...
	throws InterruptedException
		...
		while (iRetries > 0) {
			try {
				String url = "http://112.213.84.107/recv?";
				
				url = Constants._prop.getProperty("url_livesms", url);
				
				String msisdn=msgObject.getUserid();
				String sms=msgObject.getUsertext();
				String service_id=msgObject.getServiceid();
				
			    url=url+"msisdn="+msisdn+"&sms="+sms+"&service_id="+service_id;
