...
public class SoapQMChat extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		while (iRetries > 0) {
			try {
				if  ("BEELINE".equalsIgnoreCase(msgObject.getMobileoperator().toUpperCase())){
					msgObject
							.setUsertext("Yahoo Messenger:http://wap.vietnetsms.vn/chat/chat.html");
					msgObject.setContenttype(8);
					Util.logger.info("Thong tin tra ve cho khach hang BEELINE:Yahoo Messenger:http://wap.vietnetsms.vn/chat/chat.html");
	...
	private static String getValue(String xml, String tagName) {
		...
		return (f > l) ? "1" : xml.substring(f, l);
	}
