...
public class Daugia308 extends ContentAbstract {
	...
	private String[] checkSession() {
		...
		try {
			...
			String query2 = "select * from daugia_phien where current_timestamp >"
			...
			String query5 = "select begintime,endtime,sanpham from daugia_phien where phien="
					+ phien;
	...
	private String[] getWinner(int phien) {
		...
		try {
			...
			String query1 = "select min(price)from (select price,count(*) from (select * from daugia_sieure where phien="
					+ phien
		...
		try {
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					String query3 = "select count(*) from daugia_sieure where price ="
				...
				String query4 = "select count(*) from daugia_sieure where price <"
						+ price + " and phien=" + phienid;
	...
	private HashMap Banggia(int phien) {
		...
		try {
			...
			String query = "select distinct price from daugia_sieure where phien ="
	...
	private int saveWinner(String keyword, String serviceid, String userid,
		...
		try {
			...
			String sqlInsert = "Insert into daugia_winner (command_code,user_id,service_id,info,phien) values ('"
					+ keyword
					+ "','"
					+ userid
					+ "','"
	...
	private static boolean updateWinner(String keyword, String serviceid,
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_winner  SET command_code ='"
	...
	private static boolean updateDuynhat(int phien, int price) {
		...
		try {
			...
			String sqlUpdate = "UPDATE daugia_phien  SET duynhat ='" + price
					+ " ' WHERE phien=" + phien;
			Util.logger.info(" UPDATE DATE: " + sqlUpdate);
			statement = connection.prepareStatement(sqlUpdate);
			if (statement.execute()) {
	...
	private static boolean deleteWinner(int phien) {
		...
		try {
			...
			String sqlUpdate = "DELETE FROM daugia_winner  WHERE phien="
					+ phien;
