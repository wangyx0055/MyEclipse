...
public class RunClassDaysotinhcoHCM extends Thread {
	...
	private Hashtable getTime() {
		...
		try {
			...
			String sqlSelect = "SELECT id, number, start_time, block1, block2, block3, block4, block5, block6, end_time, isprocess, current FROM icom_dayso_confighcm WHERE active = 1";
			...
			if (statement.execute()) {
				...
				while (rs.next()) {
					...
					secretNumber.put("number", result[1]);
					...
					secretNumber.put("end", result[9]);
					result[10] = rs.getString(11);
					secretNumber.put("isprocess", result[10]);
					result[11] = rs.getString(12);
					secretNumber.put("current", result[11]);
				}
