...
public class DrThanh extends ContentAbstract {

	private static final String DBCONTENT = "icom_drthanh_config";
	private static final String DBCUSTOMER = "icom_drthanh_customer";
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			String _mtInvalid = "Ban nhan tin sai cu phap.Soan tin "
					...
					+ " gui "
					+ sServiceid
					+ " de binh chon cho loi chuc hay nhat trong ngay va gianh giai phu cua chuong trinh.DTHT:19001745";
					...
					if (!checkCode(sCodes, code)) {
						...
						msgObject
								.setUsertext("Ban da binh chon cho giong hat so "
										...
										+ position
										+ ".Chuc ban gianh duoc giai phu cua chuong trinh.DTHT:19001745");
						msgObject.setContenttype(0);
						msgObject.setMsgtype(1);
						messages.add(new MsgObject(msgObject));
						return messages;
	...
	private String getCode(String database, Timestamp timeReceive) {
		...
		try {
			...
			String sqlSelect = "SELECT wish_id FROM " + database
					+ " WHERE date(date_time) = date('" + timeReceive + "')";
