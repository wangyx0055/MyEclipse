...
public class NhanDien extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)
		...
		try {
			...
			//
			String mtContent = "";
			Threadnhandien nhandien = new Threadnhandien();
			Hashtable numberDelay = nhandien.lastestnumber.SECRET_NUMBER;
			nhandien.start();
			...
			String answer1 = getSECRET_NUMBER(numberDelay, "answer1");
			Util.logger.info("Answer1 :" + answer1);
			String answer2 = getSECRET_NUMBER(numberDelay, "answer2");
			Util.logger.info("Answer2 :" + answer2);
			String answer3 = getSECRET_NUMBER(numberDelay, "answer3");
			Util.logger.info("Answer3 :" + answer3);
			String answer4 = getSECRET_NUMBER(numberDelay, "answer4");
			Util.logger.info("Answer4 :" + answer4);
			String answer5 = getSECRET_NUMBER(numberDelay, "answer5");
			Util.logger.info("Answer5 :" + answer5);
			String start_question1 = getSECRET_NUMBER(numberDelay, "start_question1");
			Util.logger.info("start_question1 :" + start_question1);
			String question2 = getSECRET_NUMBER(numberDelay, "question2");
			Util.logger.info("question2 :" + question2);
			String question3 = getSECRET_NUMBER(numberDelay, "question3");
			Util.logger.info("question3 :" + question3);
			String question4 = getSECRET_NUMBER(numberDelay, "question4");
			Util.logger.info("question4 :" + question4);
			String question5 = getSECRET_NUMBER(numberDelay, "question5");
			Util.logger.info("question5 :" + question5);
			String end_question = getSECRET_NUMBER(numberDelay, "end_question");
			Util.logger.info("end_question :" + end_question);
			...
			if (Threadnhandien.lastestnumber.isProcess == 0) {
				...
				if (sTokens.length < 2) {
					...
					if (guess1.equalsIgnoreCase(nameIcon)) {
						saveCustomer(database_customer, session, user_id,
								...
								sKeyword);
						msgObject.setUsertext("Ban da du doan than tuong la "
										+ guess1
				...
				String[] sTokens = info.split(" ");
				if (sTokens.length < 2) {
					mtContent = "Tin nhan khong hop le.De tra loi cac cau hoi cua Nhan dien than tuong,soan tin"
							+ sKeyword
							+ " <dapan> gui "
							+ service_id
					...
					// Xem dang o block thu may
					String status1 = "1";
					if ((start_question1.compareToIgnoreCase(start_time) < 0)
							|| (start_question1.compareToIgnoreCase(receive_date
									.toString()) > 0)) 
					{						
						status1 = "1";
						if(guess.equalsIgnoreCase(answer1))
						{
							mt = "Chuc mung ban da tra loi dung cau hoi phu thu 1.Hay tiep tuc theo doi chuong trinh de biet ban co phai la nguoi dau tien tra loi dung khong nhe!DTHT:1900571566.";
							exactly =1;
						}
						else
						{
							mt = "Rat tiec!Dap an ban chon chua dung.Phan qua hap dan van dang cho ban.Hay nhanh tay soan tin de tro thanh nguoi dau tien doan dung nhe!DTHT:1900571566" ;
							exactly =0;
						}
					
					} else if ((question2.compareToIgnoreCase(start_time) < 0)
							|| (question2.compareToIgnoreCase(receive_date
									.toString()) > 0)) {
						status1 = "2";
						if(guess.equalsIgnoreCase(answer1))
						{
							mt = "Chuc mung ban da tra loi dung cau hoi phu thu 2.Hay tiep tuc theo doi chuong trinh de biet ban co phai la nguoi dau tien tra loi dung khong nhe!DTHT:1900571566.";
							exactly =1;
						}
						else
						{
					...
					} else if ((question3.compareToIgnoreCase(start_time) < 0)
						|| (question3.compareToIgnoreCase(receive_date
								.toString()) > 0)) {
						status1 = "3";
						if(guess.equalsIgnoreCase(answer1))
						{
							mt = "Chuc mung ban da tra loi dung cau hoi phu thu 3.Hay tiep tuc theo doi chuong trinh de biet ban co phai la nguoi dau tien tra loi dung khong nhe!DTHT:1900571566.";
							exactly = 1;
						}
						else
						{
					...
					else if ((question4.compareToIgnoreCase(start_time) < 0)
							|| (question4.compareToIgnoreCase(receive_date
									.toString()) > 0)) {
							status1 = "4";
							if(guess.equalsIgnoreCase(answer1))
							{
								mt = "Chuc mung ban da tra loi dung cau hoi phu thu 4.Hay tiep tuc theo doi chuong trinh de biet ban co phai la nguoi dau tien tra loi dung khong nhe!DTHT:1900571566.";
								exactly =1 ;
							}
							else
							{
						...
						if(guess.equalsIgnoreCase(answer1))
						{
							mt = "Chuc mung ban da tra loi dung cau hoi phu thu 5.Hay tiep tuc theo doi chuong trinh de biet ban co phai la nguoi dau tien tra loi dung khong nhe!DTHT:1900571566.";
							exactly =1;
						}
						else
						{
						...
						saveCustomerVote(database_vote, session, user_id,
								mobile_operator, guess, exactly, request, sKeyword,status1);
						return messages;
