 ...
 * @version 1.0
 */
public class couplechat_vovhn extends ContentAbstract {
	...
	protected Collection getMessages(MsgObject msgObject, Keyword keyword)throws Exception {
		try {			
			...
			if(commandCode.equalsIgnoreCase("NR"))
				{						
					try
						{
						...
						String content = "Tin nhan tu " + nickSend + " tren Radio Love: " + contentGet +" ";
						...
						if( checkUserID == true)
						{
							if (checkNickNew == true) //  nick duoc nhan co ton tai 
							{
								...
								dbInsert.insertToVovhn_chat_couple(userID, nickGet, msgObject.getTTimes().toString(), contentSend,sess);
								...
								boolean check1 = dbSelect.checkVovhn_chat_couple(userID);
								if( check1 == false)
								{
						...
						{
							msgObject.setUsertext("Ban chua dang ky nick chat tren he thong Radio Love.Soan RL <ten nick toi da 20 ky tu> <gioi tinh(nu)(nam)> <so thich> 8751. DTHT:1900571566");
							msgObject.setMsgtype(1);
							msgObject.setContenttype(0);
							DBUtil.sendMT(msgObject);
