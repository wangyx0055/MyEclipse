...
public class Ketnoi extends ContentAbstract {
		...
		try {
			...
			if (sTokens.length < 2) {
				...
				if (saveClient(msgObject.getUserid(), stt, sTokens[1],
					...
					Thread.sleep(1000);
					msgObject
							.setUsertext("Soan tin BA maso noidungchat gui 8351 de ket ban giao luu va chatting voi cac thanh vien khac cua KNYT. DT ho tro 1900571566");
				...
				if (phien == -1) {
					...
					msgObject
							.setUsertext("Ban da vote thanh cong cho cap doi so "
									+ sTokens[1]
									+ " hay doi den cuoi chuong trinh de biet ket qua chính xác. DT ho tro 1900571566");
					msgObject.setMsgtype(1);
					DBUtil.sendMT(msgObject);
					Thread.sleep(1000);
					return null;
