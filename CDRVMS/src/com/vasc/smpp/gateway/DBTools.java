...
import com.vasc.smpp.cdr.FtpData;
public class DBTools {
  ...
  public BigDecimal add2SMSSendQueue(String userId, String serviceId,
                                     String mobileOperator,
                                     String commandCode, String info,
                                     int numberOfSend,
                                     int processResult, int messageType,
                                     BigDecimal requestId,
                                     int totalSegments, int segmentSeqnum,
                                     int moreMsgsToSend) {
    BigDecimal id = null;
    ...
    try {
      ...
      strSQL =
          "INSERT INTO SMS_SEND_QUEUE (ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR, COMMAND_CODE," +
  ...
  public BigDecimal add2SMSSendQueueEx(String userId, String serviceId,
                                       String mobileOperator,
                                       String commandCode, String info,
                                       int numberOfSend,
                                       int processResult, int messageType,
    ...
    try {
      ...
      if (stmt.executeUpdate() < 1) {
        conn.rollback();
        System.out.println(
            "DBTools.addToSMSSendQueue(): Error adding row");
        return null;
      }
...
//==========================================================================
  public BigDecimal add2EMSSendQueue(String userId, String serviceId,
                                     String mobileOperator, String commandCode,
                                     int contentType, String text, byte[] bytes,
    ...
    try {
      ...
      Timestamp time = new Timestamp(System.currentTimeMillis());
      strSQL =
          "INSERT INTO EMS_SEND_QUEUE (ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR, " +
          "COMMAND_CODE, CONTENT_TYPE, INFO, RAW_INFO, MESSAGE_TYPE, REQUEST_ID) " +
          "VALUES (S_EMS_SEND_QUEUE.nextval, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      preStmt = conn.prepareStatement(strSQL);
      preStmt.setString(1, userId);
      preStmt.setString(2, serviceId);
      preStmt.setString(3, mobileOperator);
      ...
      if (preStmt.executeUpdate() != 1) {
        ...
        if (rs.next()) {
          ...
          conn.rollback();
          System.out.println(
              "DBTools.add2EMSSendQueue(): Error in S_EMS_SEND_QUEUE.currval !!!");
        }
      }
      conn.setAutoCommit(true);
    }
  ...
  public Collection getAllSMSesInSendQueue() throws DBException {
    ...
    try {
      ...
      strSQL =
          "SELECT ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR, COMMAND_CODE, INFO, " +
          "FIRST_SEND_TIME, LAST_SEND_TIME, NUMBER_OF_SEND, PROCESS_RESULT, " +
          "MESSAGE_TYPE, REQUEST_ID, TOTAL_SEGMENTS, SEGMENT_SEQNUM, " +
          "MORE_MSGS_TO_SEND, MESSAGE_ID FROM SMS_SEND_QUEUE WHERE " +
          whereClause;
      preStmt = conn.prepareStatement(strSQL);
      ...
      System.out.println("Error executing SQL " + strSQL + ">>>" + e.getMessage());
      if (e.getMessage().startsWith("ORA-03114")) { //ORA-03114: not connected to ORACLE
  ...
  //***************************************************************************
   //                     CDR LOG
   //***************************************************************************
    public BigDecimal add2CdrQueue(String userId, String serviceId,
                                   String mobileOperator, String commandCode,
                                   String info, String submitDate,
                                   String doneDate, int totalSegments) throws
        DBException {
      ...
      try {
        ...
        Timestamp time = new Timestamp(System.currentTimeMillis());
        strSQL =
            "INSERT INTO CDR_QUEUE (ID, USER_ID, SERVICE_ID, " +
            "MOBILE_OPERATOR, COMMAND_CODE, INFO, " +
            "SUBMIT_DATE, DONE_DATE, TOTAL_SEGMENTS) " +
        ...
        if (preStmt.executeUpdate() != 1) {
          ...
          if (rs.next()) {
            ...
            conn.rollback();
            System.out.println(
                "DBTools.add2CdrQueue(): Error in S_CDR_QUEUE.currval !!!");
          }
        }
        conn.setAutoCommit(true);
      }
  ...
  private String[] chargeFreeList = {
      "912099902", "904060007", "912492727", //baopv
      "903200003", "903256275", "918077770",
      "989152696", "958867571"
  ...
  public BigDecimal add2CdrQueue(SMSData sms) throws DBException {
    ...
    try {
      ...
      preStmt = conn.prepareStatement(strSQL);
      preStmt.setString(1,
                        sms.getUserIdEx(Constants.USERID_FORMAT_INTERNATIONAL));
      preStmt.setString(2,
                        sms.getServiceIdEx(Constants.
                                           SERVICEID_FORMAT_INTERNATIONAL));
  ...
  public BigDecimal add2CdrQueueEx(String userID, String serviceID,
                                   String mobileOperator, String commandCode,
                                   String info,
                                   String submitDate, String doneDate,
                                   int totalSegments) throws DBException {
    /*
             for(int i=0; i<chargeFreeList.length; i++) {
        if (userID.endsWith(chargeFreeList[i])) {
            return new BigDecimal(0);
        }
             }
     */
    ...
    try {
      ...
      preStmt.setBigDecimal(1, id);
      preStmt.setString(2,
                        SMSData.formatUserId(userID,
                                             Constants.
                                             USERID_FORMAT_INTERNATIONAL));
      preStmt.setString(3,
                        SMSData.formatServiceId(serviceID,
                                                Constants.
                                                SERVICEID_FORMAT_INTERNATIONAL));
      preStmt.setString(4, mobileOperator);
      preStmt.setString(5, commandCode);
  ...
  public BigDecimal add2CdrLog(CDR cdr) {
    ...
    try {
      ...
      conn.setAutoCommit(false);
      /*stmt = conn.prepareStatement("Select S_CDR_LOG.nextval from dual");
           if (stmt.executeUpdate() < 1) {
            conn.rollback();
             System.out.println(
           "DBTools.add2CdrLog(): Error in S_CDR_LOG.currval !!!");
       return null;
           }
             rs = stmt.getResultSet();
             if (rs.next()) {
       ...
       "DBTools.addToSMSSendQueue(): Error in S_SMS_SEND_QUEUE.currval !!!");
        return null;
             ...
             Timestamp time = new Timestamp(System.currentTimeMillis());
       */
      String tableCDR_LOG = "cdr_log" + DateProc.Timestamp2YYYYMMDD(DateProc.
          createTimestamp(), "").substring(0, 6);

      id = cdr.getId();
      strSQL =
          "INSERT INTO " + tableCDR_LOG + "  ( USER_ID, SERVICE_ID, " +
          "MOBILE_OPERATOR, COMMAND_CODE, INFO, " +
          "SUBMIT_DATE, DONE_DATE, TOTAL_SEGMENTS,process_result,MESSAGE_TYPE,REQUEST_ID,CPID) " +
          "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?)";
...
//      preStmt.setString(7, cdr.getSubmitDate());
  ...
  public CDR getCDRinQueue(BigDecimal queueId) {
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "SELECT USER_ID, SERVICE_ID, MOBILE_OPERATOR, " +
          "COMMAND_CODE, INFO, SUBMIT_DATE, DONE_DATE, " +
      ...
      System.out.println("DBTools.getCDRinQueue(id=" + queueId + "): " +
                         e.getMessage());
    }
    finally {
      GatewayCDR.dbPool.add(conn);
  ...
  public Collection getAllCDRsInQueueEx() throws DBException {
    ...
    try {
      ...
      strSQL = "SELECT ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR,  " +
          ...
          " TOTAL_SEGMENTS , process_result,Message_Type ,SUBMIT_DATE SUBMIT_DATE_TIMESTAMP , DONE_DATE DONE_DATE_TIMESTAMP,REQUEST_ID,CPID FROM cdr_queue  where MOBILE_OPERATOR='VMS'  and  USER_ID not in (select PhoneNumber from _cc_phone ) and (Message_Type like '2%'  or Message_Type ='1'  or Message_Type ='3')  " ;
    //  strSQL = strSQL.toLowerCase();
  ...
  public Collection getAllCDRsInQueueALL() throws DBException {
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "SELECT ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR, " +
          "COMMAND_CODE, INFO, SUBMIT_DATE, DONE_DATE, " +
  ...
  public Collection getAllCDRsInQueue(String mobileOperator) throws DBException {
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "SELECT ID FROM CDR_QUEUE WHERE UPPER(MOBILE_OPERATOR) = ?";
      preStmt = conn.prepareStatement(strSQL);
      ...
      System.out.println("DBTools.getAllCDRsInQueue(" + mobileOperator + "): " +
                         e.getMessage());
    }
    finally {
      GatewayCDR.dbPool.add(conn);
  ...
  public Map getAllCDRsInLog(String mobileOperator, String serviceId,
                             String yymmdd) {
    java.sql.Connection conn = null;
    PreparedStatement preStmt = null;
    ResultSet rs = null;
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "select count(*), COMMAND_CODE from CDR_LOG WHERE" +
          " UPPER(MOBILE_OPERATOR) = '" + mobileOperator.toUpperCase() +
          "' and SERVICE_ID like '%" + serviceId +
          "' and SUBMIT_DATE like '" + yymmdd +
      ...
      System.out.println("DBTools.getAllCDRsInLog(" + mobileOperator + "): " +
                         e.getMessage());
    }
    finally {
      GatewayCDR.dbPool.add(conn);
  ...
  //***************************************************************************
   ...
   //***************************************************************************
     ...
     * @param shortCode: 996, 997, 998
     */
    public boolean add2MobileProfile(String soTB84, Timestamp lastChargedTime,
                                     String shortCode) throws DBException {
      DBException exception = null;
      ...
      try {
        conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
        strSQL =
            "INSERT INTO MOBILE_PROFILE  (SOTB84, " +
            "LAST_CHARGED_TIME, LAST_CHARGED_SHORTCODE) " +
            "VALUES (?, ?, ?)";
        preStmt = conn.prepareStatement(strSQL);
  ...
  public boolean updateMobileProfile(MobileProfile profile) {
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "UPDATE MOBILE_PROFILE SET LAST_CHARGED_TIME = ?, " +
          "LAST_CHARGED_SHORTCODE = ? WHERE SOTB84 = ?";
      preStmt = conn.prepareStatement(strSQL);
      ...
      if (preStmt.executeUpdate() < 1) {
        System.out.println("DBTools.updateMobileProfile(" + profile.getSoTB84() +
                           ") Row does not exist");
      }
      else {
        result = true;
      ...
      System.out.println(
          "Error executing SQL " + strSQL + ">>>" + e.getMessage());
    }
    catch (Exception e) {
  ...
  public MobileProfile getMobileProfile(String soTB84) {
    ...
    try {
      conn = (java.sql.Connection) GatewayCDR.dbPool.remove();
      strSQL =
          "select LAST_CHARGED_TIME, LAST_CHARGED_SHORTCODE " +
          "from MOBILE_PROFILE where SOTB84 = ?";
      preStmt = conn.prepareStatement(strSQL);
      preStmt.setString(1, soTB84);
      rs = preStmt.executeQuery();
      while (rs.next()) {
      ...
      System.out.println("DBTools.getMobileProfile(" + soTB84 + ") " +
                         e.getMessage());
    }
    finally {
      GatewayCDR.dbPool.add(conn);
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      BigDecimal id = dbTools.add2CdrQueue("0904060008", "84996", "VMS", "DA",
                                           "DA 123", "0402011040", "0402011040",
                                           1);
  ...
  public static int log_alert(String domain, String alertmsg, int alert_status,
                              int checked, String level, String contactInfo) throws
      DBException {
    ...
    DBPool dbpool = new DBPool();
    String sSQL =
        "insert into msg_alerter(domain,alertmsg,alert_status,checked,level,contact) " +
        "values(?,?,?,?,?,?)";
    try {
      //connection = util.getDBConnectionAlert(Preference.db_DriverClassName_alert,
      ...
      if (ex.getMessage().startsWith("ORA-03114")) { //ORA-03114: not connected to ORACLE
   ...
   * For pushCDR to file
   */
  public Collection getAllCDRsInLogEx8x99(String fromNumber, String toNumber) throws
      DBException {
    DBException exception = null;
    java.sql.Connection conn = null;
    PreparedStatement preStmt = null;
    ...
    Vector keys = new Vector();
    try {
      conn = (java.sql.Connection) GatewayCDR.util.getDBConnection(
          "oracle.jdbc.driver.OracleDriver", "jdbc:oracle:oci8:@mcdb27",
          "vmsgw", "smsadmin12345");
      strSQL =
          "SELECT ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR, " +
          "COMMAND_CODE, INFO, SUBMIT_DATE, DONE_DATE, " +
          "TOTAL_SEGMENTS FROM CDR_LOG WHERE SERVICE_ID LIKE'%99' AND LENGTH(SERVICE_ID)<7 AND TO_NUMBER(SUBMIT_DATE) BETWEEN " +
