...
public abstract class ProcessingThread extends SmppObject implements Runnable
{
    ...
    public void start()
    {
    	Logger.verbose(DUTL,this.getThreadName()+ "start()");
        if (!isProcessing()) { // i.e. is initialising or finished
            ...
            while (isInitialising()) {
            	Logger.verbose("we're waiting for the proc thread to start");
    ...
    public void stop()
    {
    	Logger.verbose(DUTL,this+"stop()");
        if (isProcessing()) {
            stopProcessing(null);
            while (!isFinished()) {
            	Logger.verbose("isFinished");
    ...
    public void run()
    {
    	Logger.verbose(DUTL,"ProcessingThread.run-->run()");
        try {
            setProcessingStatus(PROC_RECEIVING);
            while (keepProcessing) {
            	Logger.verbose("ProcessingThread.run-->keepProcessing:" + getThreadName());
                process();
                Thread.yield();
                Logger.verbose("ProcessingThread.run-->keepProcessing.run():"  + getThreadName());
            }
        } catch (Exception e) {
            setTermException(e);
            Logger.verbose("ProcessingThread.run() caught unhadled exception "+e);
            
        } finally {
            setProcessingStatus(PROC_FINISHED);
            //debug.exit(DUTL,this);
        }
        Logger.verbose(DUTL,"ProcessingThread.run-->finish()");
    }
