...
public class LoadRepeat extends Thread
{
	...
	public boolean isRepeat(String userid)
	{
		...
		try
		{
			...
			{
				Logger.info("{Loadrepeat.isRepeat}{Keytosearh=" + userid + "} {repeat=true}");
				return true;
			}
		}
		catch (Exception e)
		...
		Logger.info("{Loadrepeat.isRepeat}{Keytosearh=" + userid + "} {repeat=false}");
		return false;
	...
	public static Hashtable retrieveRepeatlist() throws Exception
	{
		...
		String query = "select user_id from " + tableblacklist + "  where upper(mobile_operator) = '"
		...
		catch (SQLException ex2)
		{
			...
			DBTools.ALERT("LoadRepeat", "LoadRepeat", Constants.ALERT_MAJOR, Preference.Channel + ex2.getMessage(),
		...
		catch (Exception ex3)
		{
			...
			DBTools.ALERT("LoadRepeat", "LoadRepeat", Constants.ALERT_MAJOR, Preference.Channel + ex3.getMessage(),
					Preference.ALERT_CONTACT);
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
			...
			catch (Exception ex3)
			{
				...
				Logger.printStackTrace(ex3);
				DBTools.ALERT("Load repeat error", "Load repeat error", Constants.ALERT_SERIOUS, Preference.Channel
						+ "Load config error: " + ex3.getMessage(), Preference.ALERT_CONTACT);
