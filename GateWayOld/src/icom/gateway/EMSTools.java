...
public class EMSTools
{
			...
			throws EMSException
	{
		 ...
		 * mobileOperator = "VIETTEL"; } else if (destAddr.startsWith("8495")) {
		...
		try
		{
			...
			switch (contentType)
			{
				...
				case Constants.CT_OPER_LOGO :
					 ...
					 * logo.setMobileOperator("VIETTEL"); } else if
	...
	public static String[] splString(String strInput, int strLen)
	{
		try
		{
			...
			for (int i = 0; i < strInput.length(); i++)
			{
				...
				if (strtemp[itemp].length() >= strLen) itemp++;
			...
			BigDecimal emsId, byte registeredDelivery, int MsgType, String commandcode) throws EMSException
	{
		...
		try
		{
			...
			for (int i = 0; i < totalSegs; i++)
			{
				...
				// IE Data as 3 octets
				// 1. Ref number for this concatenated message, 0 decimal
				// linhnc sua lai: 5/5/2014
				message.appendByte((byte) Integer.parseInt(s_emsId));
				// message.appendByte((byte) 0x00);
				// 2. # of segments
