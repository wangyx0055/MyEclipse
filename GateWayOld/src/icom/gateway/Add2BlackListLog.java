...
public class Add2BlackListLog extends Thread
{
	...
	public void run()
	{
		...
		while (Gateway.running)
		{
			...
			catch (Exception ex)
			{
				try
				{
					Gateway.util.log(this.getClass().getName(), "AddMT2BlacklistLog:" + ex.getMessage());
				}
				catch (Exception e)
				{
				}
	...
	public void AddMT2BlacklistLog()
	{
		...
		try
		{
			...
			if (logID1 != 0)
			{
				Gateway.util.log(
						...
						"{add2BlacklistLog}{User_ID=" + sms.getUserId() + "}{Service_ID=" + sms.getServiceId()
								+ "}{Info=" + sms.getText() + "}{Request_ID" + sms.getsRequestID() + "}{emsID="
								+ sms.getId() + "}");
			...
			else
			{
				Gateway.util.logErr(
						this.getClass().getName(),
						"{ERR: add2BlacklistLog}{User_ID=" + sms.getUserId() + "}{Service_ID=" + sms.getServiceId()
								+ "}{Info=" + sms.getText() + "}{Request_ID" + sms.getsRequestID() + "}{emsID="
								+ sms.getId() + "}");
			}
		}
		...
		{
			Gateway.util.logErr(this.getClass().getName(), "{ERR: add2BlacklistLog}{User_ID=" + sms.getUserId()
					+ "}{Service_ID=" + sms.getServiceId() + "}{Info=" + sms.getText() + "} ERR=" + ex.getMessage());
