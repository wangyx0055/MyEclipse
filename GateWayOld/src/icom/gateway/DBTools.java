...
public class DBTools
{
	...
	public static int add2SMSReceiveQueueR(String userId, String serviceId, String mobileOperator, String commandCode,
			String info, String RequestID, String DestinationPort) throws DBException
	{
		...
		if (!Preference.CheckServiceId(serviceId)) { return 1; }
		try
		{
			...
			if (preStmt.executeUpdate() != 1)
			{
				...
				DBTools.ALERT("DBTools", "add2SMSReceiveQueue", Constants.ALERT_WARN, Preference.Channel
						+ "@Error adding row", Preference.ALERT_CONTACT);
			...
			else
			{
				...
				Logger.info("DBTools.add2SMSReceiveQueue", "{Add-MO2DB}{Request_ID=" + RequestID + "}{User_ID="
						+ userId + "}{Service_ID=" + serviceId + "}{Info=" + info + "}");
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2SMSReceiveQueue", Constants.ALERT_WARN, Preference.Channel
					+ "@Error adding row:" + e.getMessage(), Preference.ALERT_CONTACT);
	...
	public static int ALERT(String domain, String issue, String level, String alertmsg, String contact)
	{
		...
		Logger.info("ALERT:" + "{domain=" + domain + "}{issue=" + issue + "}{level=" + level + "}{domain=" + level
				+ "}{alertmsg=" + alertmsg + "}{contact=" + contact + "}");
		if (Gateway.running != true) { return 1;
		// Gateway.running
		}
		...
		if (Preference.ALERT == 1)
		{
			...
			try
			{
				if (connection == null) connection = getConnection(Preference.LIST_DB_ALERT[0].trim());
				...
				if (stmt.executeUpdate() == -1) { return -1; }
				return 1;
			}
			catch (Exception ex)
			{
	...
	public boolean getAllEMSSendQueue(String sRound, String iMod, String iNum, Queue Ems, int idDB) throws DBException
	{
		...
		try
		{
			...
			strSQL = "select id, user_id, service_id, mobile_operator, command_code, content_type, "
					+ "info,  process_result, message_type, " + "request_id, message_id, total_segments,cpid "
					+ "from ems_send_queue" + sR + " where  info is not null " + "and (mod(id," + iMod + ") = " + iNum
					+ ") and upper(mobile_operator)=upper('" + Preference.mobileOperator + "') and service_id in ("
			...
			if (preStmt.execute())
			{
				...
				while (rs.next())
				{
					...
					if (Preference.CheckServiceId(serviceid) & Gateway.bound)
					{
						...
						try
						{
							Logger.info(this.getClass().getName(), "{getMTfromDB}{Request_ID=" + ems.getRequestId()
									+ "}{User_ID=" + ems.getUserId() + "}");
							rs.deleteRow();
							Ems.enqueue(ems);
						...
						catch (Exception ex)
						{
							Logger.error(
									...
									"{getAllEMSSendQueue: Error executing SQL }{getMTfromDB}{Request_ID="
											+ ems.getRequestId() + "}{User_ID=" + ems.getUserId() + "}" + strSQL + ": "
											+ ex.toString());
							DBTools.ALERT("DBTools", "getAllEMSSendQueue", Constants.ALERT_WARN, Preference.Channel
									+ "@getAllEMSSendQueue:" + ex.getMessage(), Preference.ALERT_CONTACT);
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "getAllEMSSendQueue", Constants.ALERT_WARN, Preference.Channel
					+ "@getAllEMSSendQueue:" + e.getMessage(), Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
	...
	public int add2EMSSendLog(EMSData ems)
	{
		...
		try
		{
			...
			strSQL = "insert into " + tablename + " ( USER_ID, SERVICE_ID, MOBILE_OPERATOR, "
		...
		catch (SQLException e)
		{
			Logger.error(this.getClass().getName(),
					"SQLException: Error executing SQL " + strSQL + ">>>" + e.getMessage());
			Gateway.saveSMSObject(ems.getId() + ".slog", ems);
			DBTools.ALERT("DBTools", "add2EMSSendLog", Constants.ALERT_WARN, Preference.Channel + "@add2EMSSendLog:"
	...
	public int add2BlacklistLog(EMSData ems)
	{
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2BlacklistLog", Constants.ALERT_WARN, Preference.Channel
					+ "@add2BlacklistLog:" + e.getMessage(), Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
			return result;
		}
		catch (Exception e)
	...
	public static boolean add2CdrQueueb(EMSData ems, String MessageType) throws DBException
	{
		...
		if (ems.getMobileOperator().equalsIgnoreCase("SFONE")) { return add2CdrLogb(ems, MessageType); }
		DBException exception = null;
		boolean rt = false;
		java.sql.Connection conn = null;
		PreparedStatement preStmt = null;
		...
		try
		{
			...
			else
			{
				...
				DBTools.ALERT("DBTools", "add2BlacklistLog", Constants.ALERT_WARN, Preference.Channel
						+ "@Cannot add CDRQueue", Preference.ALERT_CONTACT);
		...
		catch (SQLException e)
		{
			...
			DBTools.ALERT("DBTools", "add2CdrQueueb", Constants.ALERT_WARN,
					Preference.Channel + "@add2CdrQueueb:" + e.getMessage(), Preference.ALERT_CONTACT);
			Logger.printStackTrace(e);
	...
	public static boolean add2CdrLogb(EMSData ems, String MessageType) throws DBException
	{
		...
		try
		{
			...
			conn.setAutoCommit(false);
			strSQL = "insert into cdr_log" + new SimpleDateFormat("yyyyMM").format(new Date())
