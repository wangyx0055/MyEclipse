			...
			try {
				if (Wait4ResponseTable.size() > 0) {
					...
					synchronized (Wait4ResponseTable) {
						...
						for (Enumeration e = new Vector(Wait4ResponseTable
								.keySet()).elements(); e.hasMoreElements();) {
							...
							if (isTimeOut(ems.getSubmitDate())) {
								...
								ems
										.setNotes("No ACK return for this message!!!");
				...
				Gateway.util.logErr(this.getClass().getName(),
						"CheckTimeOutWait4Response:" + ex.getMessage());
