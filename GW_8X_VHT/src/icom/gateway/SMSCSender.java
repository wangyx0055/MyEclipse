			...
			if (Gateway.bound) {
				try {
					...
					if (pdu != null) {
						if (pdu.isResponse()) {
							...
							//synchronized (this) {
								...
								int time2sleep = (toSMSC.size() > 20) ? 30
										: (1000 / Preference.maxSMPerSecond);
						...
						DBTools.ALERT("SMSCSender", "pdu is null",
					...
					Gateway.util.log(this.getClass().getName(),
							"InterruptedException:" + ex1.getMessage());
					DBTools.ALERT("SMSCSender", "InterruptedException",
							Constants.ALERT_WARN, Preference.Channel
					...
					Gateway.util.log(this.getClass().getName(),
							"ValueNotSetException:" + ex.getMessage());

					DBTools
							.ALERT("SMSCSender", "ValueNotSetException",
									Constants.ALERT_MAJOR, Preference.Channel
											+ "ValueNotSetException: "
											+ ex.getMessage(),
									Preference.ALERT_CONTACT);

				} catch (IOException ex) {
					Gateway.util.log(this.getClass().getName(), "IOException:"
							+ ex.getMessage());
					toSMSC.enqueue(pdu); // Connection error; back to queue.

					DBTools.ALERT("SMSCSender", "IOException",
					...
					Gateway.util.log(this.getClass().getName(), "Exception:"
							+ ex.getMessage());
					toSMSC.enqueue(pdu);

					DBTools.ALERT("SMSCSender", "Exception",
							Constants.ALERT_SERIOUS, Preference.Channel
	...
	private void sendRequest(PDU pdu) throws IOException {
		...
		if (pdu.getCommandId() == Data.SUBMIT_SM) {
			try {
				...
				Gateway.util.log(this.getClass().getName(), "{toSMSC size= "
						+ toSMSC.size() + "}{User_ID= "
						+ request.getDestAddr().getAddress() + "}{Service_ID="
						+ request.getSourceAddr().getAddress() + "}{Info = "
						+ info + "}");
				...
				Gateway.util.log(this.getClass().getName(),
						"WrongSessionStateException: " + ex.getMessage());

				DBTools.ALERT("SMSCSender", "WrongSessionStateException",
						Constants.ALERT_SERIOUS, Preference.Channel
								+ "WrongSessionStateException: "
								+ ex.getMessage(), Preference.ALERT_CONTACT);
			} catch (PDUException ex) {
				Gateway.util.log(this.getClass().getName(), "PDUException: "
						+ ex.getMessage());

				DBTools.ALERT("SMSCSender", "PDUException",
						Constants.ALERT_SERIOUS, Preference.Channel
								+ "PDUException: " + ex.getMessage(),
						Preference.ALERT_CONTACT);

			} catch (TimeoutException ex) {
				Gateway.util.log(this.getClass().getName(),
						"TimeoutException: " + ex.getMessage());

				DBTools.ALERT("SMSCSender", "TimeoutException",
						Constants.ALERT_SERIOUS, Preference.Channel
								+ "TimeoutException: "
								+ ex.getMessage()
								+ "\n"
				...
				Gateway.util.log(this.getClass().getName(), "Exception1: "
						+ ex.getMessage());
