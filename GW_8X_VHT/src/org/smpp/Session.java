...
public class Session extends SmppObject
{
     ...
     */
    final public BindResponse bind(BindRequest bindReq,
                                   ServerPDUEventListener pduListener)
    throws ValueNotSetException, TimeoutException, PDUException, IOException,
           WrongSessionStateException
    {
        checkState(bindReq);
        if (bound) {
        	...
        	Logger.error(this.getClass().getName(), "^^^^^^^^exception? yes, checked above");
            return null;
        } else {
        	
        	//Logger.verbose("OPENINGGGGGGGG");
            open();
            //Logger.verbose("OPENED");
            ...
            //System.err.println("SENDD");
            bound = ((bindResp != null) &&
                     (bindResp.getCommandStatus() == Data.ESME_ROK));
            //System.err.println("RESULT:" +bindResp.getCommandStatus());
            if (!bound) {
                //receiver.stop();
                close();
            } else {
           ...
           WrongSessionStateException
    {
        ...
        if (bound) {
            ...
            if (asynchronous) {
                ...
                Logger.verbose(DSESS,"going to unbound sync session");
     ...
     * 
     * @param request The generic nack PDU.
     * 
     * @exception IOException exception during communication
     * @exception TimeoutException rest of data not received for too long time
     * @exception ValueNotSetException optional param not set but requested
           ...
           WrongSessionStateException
    {
        ...
        try {
            ...
            Logger.verbose("strange, generic nack thrown "+e);
            Logger.verbose("this shouldn't happend");
           ...
           IOException
    {
        ...
        if (receiver.isReceiver()) {
            ...
            // throw?
        	Logger.info("Session", "receiver.is not Receiver() ---------------> throw" );
        }
        return pdu;
    }
     ...
     * @see Transmitter
     */
    final public void respond(Response response)
    throws ValueNotSetException, IOException,
           WrongSessionStateException
    {
        checkState(response);
        
        Logger.verbose(DSESS,"Sending response " + response.debugString());
        try {
            transmitter.send(response);
        } catch (ValueNotSetException e) {
        	Logger.error("Sending a response.");
    ...
    throws ValueNotSetException, TimeoutException, PDUException, IOException
    {
       // debug.enter(DSESS,this,"send(Request)");
        ...
        Logger.verbose(DSESS,"Session.send-->Sending request " + request.debugString());
        try {
            transmitter.send(request);
        } catch (ValueNotSetException e) {
            Logger.error("Session.send-->Sending the request." + e.getMessage());
           // debug.exit(DSESS,this);
            throw e;
        }
        if ((!asynchronous) && (request.canResponse())) {
            ...
            try {
                Logger.verbose(DSESS,"Session.send-->Going to receive response. Expecting " +
                            expResponse.debugString());
                try {
                    pdu = receiver.receive(expResponse);
                } catch (NotSynchronousException e) {
                    Logger.verbose("Session.send-->Unexpected NotSynchronousException caught, ignoring :-)");
                }
                ...
                if ((e.getException() instanceof NotEnoughDataInByteBufferException) ||
                    ...
                    Logger.verbose(DSESS,"Session.send-->wrong length " + e);
                    Logger.verbose(DSESS," => sending gnack.");
                    safeGenericNack(Data.ESME_RINVMSGLEN,e.getPDU().getSequenceNumber());
                } else {
                    Logger.verbose(DSESS,"Session.send-->InvalidPDUException - rethrowing " + e);
                    //debug.exit(DSESS,this);
                    throw e;
                }
            } catch (TimeoutException e) {
                Logger.verbose(DSESS,"Session.send-->TimeoutException - rethrowing " + e);
               // debug.exit(DSESS,this);
                throw e;
            }
            if (pdu != null) {
                Logger.verbose(DSESS,"Session.send-->Got response(?) pdu " + pdu.debugString());
                response = checkResponse(pdu,expResponse);
            } else {
                Logger.verbose(DSESS,"Session.send-->No response received.");
            }
        }
       // debug.exit(DSESS,this);
        Logger.verbose(DSESS,"Session.send-->exit send request " + request.debugString());
        return response;
    }
    ...
    {
        Response response = null;
        Logger.verbose(DSESS,"checking response if it's what we expected.");
        if (pdu.getCommandId() != expResponse.getCommandId()) {
            Logger.verbose(DSESS,"Got different response than expected " +
                        expResponse.debugString());
            if (pdu.getCommandId() == Data.GENERIC_NACK) {
                ...
                Logger.verbose(DSESS,"Got generic nack. What could we do wrong?");
            	...
            	Logger.verbose(DSESS ,"invalid command id - sending gnack");
    ...
    throws IOException
    {
        try {
            ...
            Logger.verbose("Ignoring unexpected SmppException caught sending generic nack.");
     ...
     */
    public void enableStateChecking() { this.stateChecking = true; }

    /**
     * Disables checking if the session allows certain operation in the current state.
     */
    public void disableStateChecking() { this.stateChecking = false; }
    ...
    throws WrongSessionStateException
    {
        if (stateChecking) {
            ...
            Integer requestedStateInteger =
                pduMatrix==null ? null : (Integer)pduMatrix.get(commandIdInteger);
    ...
    implements ServerPDUEventListener
    {
        ...
        public UnbindServerPDUEventListener(Session session,
                                            ServerPDUEventListener origListener,
                                            Unbind unbindReq)
        {
        ...
        public void handleEvent(ServerPDUEvent event)
        {
            ...
            if (pdu.equals(unbindReq)) {
                synchronized (this) {
                    try {
                        unbindResp =
                            (UnbindResp)(session.checkResponse(pdu,expectedResp));
                    } catch (Exception e) {
                        Logger.verbose(DSESS,"exception handling unbind "+e);
        ...
        public UnbindResp getUnbindResp() { return unbindResp; }
