...
public class CheckMarkProcess extends ContentAbstract
{
	...
	protected Collection<MsgObject> getMessages(MsgObject msgObject, Keyword keyword) throws Exception
	{
		try
		{
			...
			{
				MTContent = Common.GetDefineMT_Message(MTType.CheckMark_NotReg);
				AddToList();
				return ListMessOject;
			}

			MTContent = Common.GetDefineMT_Message(MTType.CheckMark);

			if (MTContent.contains("{TotalMark}"))
			{
				MTContent = MTContent.replace("{TotalMark}", mTable_Sub.GetValueAt(0, "TotalMark").toString());
			}
