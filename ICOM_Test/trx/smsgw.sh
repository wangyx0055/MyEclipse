#!/bin/bash
thumucchay=/home/oracle/Gateway/trx
thamsogrep="/home/oracle/Gateway/trx"
thumuclog=/home/oracle/Gateway/log
today=`date '+%Y-%m-%d'`
r=`tput setaf 1`
r1=`tput setaf 2`
N=`tput sgr0`
...
stop(){
    ...
    while [ $STOPTIMEOUT -gt 0 ]; do
       # /bin/ps -ef |grep $thamsogrep |grep -v grep |awk '{print$2}' |xargs kill
         sleep 1
         ...
         then
         echo "Stop${r}[faild]${N}"
         echo "Timeout error khi stop" $thumucchay "va Gateway chua stop"
         echo "Su dung smsgw.sh kill de kill -9 ung dung" $thumucchay
        fi
        if [ $t3 -eq 0  ];
        then
         echo "Stop${r1}[ok]${N}"
...
taillog(){
  /usr/bin/tail -f $thumuclog/gateway$today.log 
}
...
case "$1" in
  ...
  status)
    ...
    if (( $t3 > 1  ))
    then
    echo $thumucchay "dang chay nhieu hon 1 tien trinh, phai dung smsgw.sh kill de kill -9 ung dung nay"
    fi
    ;;
  taillog)
  taillog
