...
public class CdrFileCopier4vms
    extends Thread {
      ...
      try {
        ...
        if ("Y".equals(FtpData.IS_8X)) {
          if ("SFONE".equals(Preference.mobileOperator)) {
            this.vFiles = fileTool.getAllFiles(
                new File(Preference.cdroutFolder), FILE_EXTENSION_8X);
            if (vFiles.size() > 0) {
              //1) Updates a new filename in the vms_lastcdrfile.dat
              CdrFilename4vms.setNewFilename(CdrFilename4vms.
                                             getNewFilename8x99SFONE());
        ...
        if ("SFONE".equals(Preference.mobileOperator)) {
          this.vFiles = fileTool.getAllFiles(
              new File(Preference.cdroutFolder), FILE_EXTENSION_SFONE);
          if (vFiles.size() > 0) {
            //1) Updates a new filename in the sfone_lastcdrfile.dat
          ...
          this.vFiles = fileTool.getAllFiles(
              new File(Preference.cdroutFolder), FILE_EXTENSION);
          if (vFiles.size() > 0) {
            //1) Updates a new filename in the vms_lastcdrfile.dat
            CdrFilename4vms.setNewFilename(CdrFilename4vms.
                                           getNewFilename());
...
//              }
  ...
  private boolean move2SendoutVMS() {
      for (int i = 0; i < vFiles.size(); i++) {
        ...
        //File dest_file   = new File(SENT_FOLDER + "\\" + source_file.getName());
        File dest_file = new File(Preference.cdrsentFolder + "\\" + sDateFolder +
                                  "\\" + source_file.getName());
        File vms_file = null;
        if ("Y".equals(FtpData.IS_8X)) {
          if ("SFONE".equals(Preference.mobileOperator)) {
            ...
            vms_file = new File(VMS_FOLDER + "\\" + sDateVMS + "\\" +
                                source_file.getName());
          }
        }
        else {
        ...
        try {
          ...
          System.out.println("CdrFileCopy4vms.move2Sendout error: " +
                             e.getMessage());
          return false;
        }
      }
