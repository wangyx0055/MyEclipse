...
public class CDRTool
{
	...
	public synchronized static void add2CDRFile(CDR cdr)
	{
		...
		else if ("VIETTEL".equals(cdr.getMobileOperator()))
		{
		...
		else if ("HTC".equals(cdr.getMobileOperator()))
		{
			...
			{
				fileCDR = CdrFilename4vms.getNewFilenameHTC();
			}
			try
			{
				java.io.DataOutputStream fout = new java.io.DataOutputStream(new FileOutputStream(Preference.cdroutFolder + "\\" + fileCDR, true)); // append
	...
	public synchronized static void add2CDRFileEx(CDR cdr)
	{
		...
		if ("HTC".equals(cdr.getMobileOperator()))
		{
			...
			try
			{
				...
				String TimeSend = cdr.getDoneDate();
				String TimeReceive = cdr.getSubmitDate();
				String ContCode = "103";
				String ContType = "2";
				String Desc = "hbcom-info:" + cdr.getInfo();
				...
				if ("0".equals(ProcessResult))
				{
					...
					ServiceState = "U"; // ghi cuoc hoan tien
				}
				else
				{
				...
				if (i == 0)
				{
					sData = Service_Id + "\t" + UserId + "\t" + TimeReceive + "\t" + ServiceState + "\t" + TimeSend + "\t" + ContCode + "\t"
							+ ContType + "\t" + Desc;
				}
				else
				{
					sData = "\r\n" + Service_Id + "\t" + UserId + "\t" + TimeReceive + "\t" + ServiceState + "\t" + TimeSend + "\t" + ContCode + "\t" + ContType + "\t"
							+ Desc;
				}
