	...
	public Collection getAllCDRsInQueueEx() throws DBException
	{
		...
		try
		{
			...
			strSQL = "SELECT ID, USER_ID, SERVICE_ID, MOBILE_OPERATOR,  "
					...
					+ " TOTAL_SEGMENTS , process_result,Message_Type ,SUBMIT_DATE SUBMIT_DATE_TIMESTAMP , DONE_DATE DONE_DATE_TIMESTAMP,REQUEST_ID,CPID FROM cdr_queue  where MOBILE_OPERATOR='HTC'  and  USER_ID not in (select PhoneNumber from _cc_phone ) and (Message_Type like '2%'  or Message_Type ='1'  or Message_Type ='3')  ";
			// strSQL = strSQL.toLowerCase();
